...
from prosperity4bt.data import BacktestData, get_limit, read_day_data
...
def prepare_state(state: TradingState, data: BacktestData) -> None:
    for product in data.products:
        ...
        # Prosperity 4 uses "XIRECS" as the denomination currency
        state.listings[product] = Listing(product, product, "XIRECS")

    # Build observations from data (generic, not hardcoded to any product)
    observation_rows = data.observations.get(state.timestamp)

    if not observation_rows:
        state.observations = Observation({}, {})
    else:
        conversion_observations = {}
        for obs_row in observation_rows:
            conversion_observation = ConversionObservation(
                bidPrice=obs_row.bidPrice,
                askPrice=obs_row.askPrice,
                transportFees=obs_row.transportFees,
                exportTariff=obs_row.exportTariff,
                importTariff=obs_row.importTariff,
                sugarPrice=obs_row.sugarPrice,
                sunlightIndex=obs_row.sunlightIndex,
            )
            conversion_observations[obs_row.product] = conversion_observation
