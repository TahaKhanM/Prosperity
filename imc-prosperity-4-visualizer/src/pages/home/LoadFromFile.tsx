...
export function LoadFromFile(): ReactNode {
    ...
    (files: File[]) =>
      new Promise<void>((resolve, reject) => {
        ...
        if (file.name.toLowerCase().endsWith('.zip')) {
          reject(new Error('Zip bundles are not parsed in-browser yet. Extract the official `.log` or `.json` file from the IMC zip and upload that file instead.'));
          return;
        }
  ...
  return (
    <HomeCard title="Load from file">
      <Text>
        Supports both logger-style text logs and the current official IMC payload format after extraction from the zip
        bundle. For official runs, upload the extracted <code>.log</code> or <code>.json</code> file rather than the
        zip itself.
      </Text>
