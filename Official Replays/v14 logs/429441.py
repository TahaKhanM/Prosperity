"""Round 3 v14 — v12 + multi-strike, conviction-scaled IV residual scalp.

Lineage
-------
v9   hosted +9,115     (HYD MM + VFE MM + smile MM + accumulation)
v10  hosted +9,227     (learned-anchor / wall-mid blend; +112)
v11  hosted +9,227     (delta-hedge skipped at runtime; no-op vs v10)
v12  hosted +11,654    (single-strike scalp K=5300 added; +2,539)
v14  this file         (v12 + scalp generalised; expected hosted >= v12)

What v14 changes vs v12
-----------------------
Only the residual scalp. Everything else (HYD MM, VFE MM, smile MM,
accumulation, identity-arb branches) is byte-identical to v12 so the
attribution of any hosted delta belongs to the scalp upgrade.

The hint that motivated this version: the IV-residual signal is real
(v12 hosted proved it), the scalp is the under-exploited alpha, and
sizing should reflect conviction — bigger gap, bigger position.

1. Multi-strike. v12 scalped K=5300 only. v14 scalps {5300, 5400, 5500},
   the three strikes with residual half-life >= 1.4 ticks and meaningful
   sigma * vega per smile_stability_report.md:
   ...
   K=5100/5200 are excluded: half-life ~0.77 ticks (signal reverts
   before we can fill across the spread). K=4000/4500 have zero time
   value (synthetic delta-1; quirk #2). K=6000/6500 are price-floor
   noise (smile_stability_report explicit warning).

2. Per-strike threshold. K=5300/5400 keep z=0.6 (proven on hosted via
   v12). K=5500 lifts to z=1.0 because sigma * vega is half: each
   sigma yields ~0.4 shells of expected reversion, below the spread
   cost of crossing unless conviction is higher.

3. Conviction-proportional sizing. v12 fired a flat 10 lots whenever
   |z| > 0.6. v14 scales:

       lots = clamp(BASE + SCALE * (|z| - threshold), MIN, MAX)
            = clamp(5 + 15 * excess_z, 3, 40)

   At |z| = threshold:        5 lots
   At |z| = threshold + 1.0:  20 lots
   At |z| = threshold + 2.3:  40 lots (max)

   This implements the hint directly: deviation magnitude IS the
   conviction signal, position size IS the response. Flat sizing
   throws away the difference between 0.6-sigma and 2.5-sigma events.

4. Hard per-strike inventory cap. v12 used a single SCALP_POS_CAP=25
   as a soft pre-fire check (could be overshot by one fire's size).
   v14 enforces caps as hard absolute bounds and shrinks the intended
   size to fit:  30 / 25 / 20 for K=5300 / 5400 / 5500 (sized down for
   the wing strike with weaker per-sigma edge).

What is intentionally NOT changed
---------------------------------
- HYD MM constants (anchor 10000, A_W 0.40, A_HYD 0.10, H_TAKE 4,
  H_QE 3, H_IMB 5, H_SIZE 20). v13's tuning of these claimed +30k
  on local rust BT but rust BT is not source of truth (memory rule).
  v12's hosted result validated these values.
- START_TTE_DAYS = 8.0 for backtester runs. Flip to 5.0 only when
  packaging for live R3 (TTE = 5d at round start).
- Smile MM warm-up (30 ticks) and EMA alpha (0.05).
- Per-strike static bias dict (same 30k-tick measured residual means).

Expected hosted impact
----------------------
v12 captured ~2.5k hosted from K=5300 alone with flat sizing. Going
1 strike -> 3 strikes plus conviction sizing can plausibly 2-3x that.
Worst case the wing strikes contribute zero and v14 ties v12, because
the K=5300 scalp parameters are unchanged at threshold and the new
sizing only fires *more* when |z| is large.
...
SCALP_RES_EMA_ALPHA = 0.05         # half-life ~14 ticks (carry-over from v12)

# Per-strike z trigger. K=5300/5400 keep v12's 0.6 (validated on hosted).
# K=5500 lifts to 1.0 because sigma * vega is ~0.43 shells/sigma — needs
# more conviction to cover spread.
...
def _scalp_lots_from_z(abs_z: float, threshold: float) -> int:
    ...
    Returns the number of lots to fire given |z| above the per-strike
    threshold. Bigger gap, bigger position — clamped to [MIN, MAX].
    """
...
def _residual_scalp(state: Dict, depths: Dict[str, OrderDepth],
    ...
    for k in SCALP_STRIKES:
        ...
        if z > 0:
            ...
            sell_room = max(0, pos + cap_k)             # hard cap: pos >= -cap_k
            global_room = max(0, CAP_VOU + pos)         # absolute voucher cap
            ...
            buy_room = max(0, cap_k - pos)              # hard cap: pos <= +cap_k
            global_room = max(0, CAP_VOU - pos)         # absolute voucher cap
