"""Temp Round 2 sweep variant 07: Pepper quote-side/noquote gate."""
...
def trade_pepper(
    ...
    passive_buy_gate = (
        temp_position >= 74
        and regime_progress >= 0.18
        and residual >= -0.5
        and not buyflow
    )
    if passive_buy_gate:
        bid_edge += 0.5
        ask_edge = max(3.6, ask_edge - 0.2)
    ...
    if passive_buy_gate:
        buy_sizes[0] = max(0, int(round(buy_sizes[0] * 0.5)))
        buy_sizes[1] = 0 if False else max(0, int(round(buy_sizes[1] * 0.5)))
        sell_sizes[0] += 1
    if recycle_exit_push:
