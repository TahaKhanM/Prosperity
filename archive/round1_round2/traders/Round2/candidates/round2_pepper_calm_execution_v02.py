"""Round 2 candidate: calm Pepper execution with a mild near-cap flattening bias."""
...
def trade_pepper(
    ...
    take_clip = 8 if live_spread is not None and live_spread >= 7 else 10
    clear_clip = 10 if live_spread is not None and live_spread >= 7 else 12
    if mode == "carry" and not buyflow:
        target_long = min(target_long, 76)
    if position >= 76 and residual >= 3.0 and not buyflow:
        target_long = min(target_long, 74)
        sell_edge = max(3.6, sell_edge - 0.15)
        ask_edge = max(4.8, ask_edge - 0.18)
        bid_size = max(6, bid_size - 1)
    ...
    for ask in sorted(depth.sell_orders):
        ...
        if ask > buy_ceiling:
            break
        before = buy_budget
        take_qty = min(-depth.sell_orders[ask], take_clip)
        buy_budget = add_buy(orders, PEPPER, ask, take_qty, buy_budget)
        temp_position += before - buy_budget
    ...
    for bid in sorted(depth.buy_orders, reverse=True):
        ...
        if temp_position <= target_long or bid < sell_threshold:
            break
        before = sell_budget
        take_qty = min(depth.buy_orders[bid], take_clip)
        sell_budget = add_sell(orders, PEPPER, bid, take_qty, sell_budget)
    ...
    if temp_position > long_cap:
        for bid in sorted(depth.buy_orders, reverse=True):
            if bid < fair - clear_edge:
                break
            before = sell_budget
            sell_qty = min(depth.buy_orders[bid], clear_clip)
            sell_budget = add_sell(orders, PEPPER, bid, sell_qty, sell_budget)
            temp_position -= before - sell_budget
            if temp_position <= long_cap:
                break
    ...
    if temp_position < -short_cap + 8:
        for ask in sorted(depth.sell_orders):
            if ask > fair + clear_edge:
                break
            before = buy_budget
            buy_qty = min(-depth.sell_orders[ask], clear_clip)
            buy_budget = add_buy(orders, PEPPER, ask, buy_qty, buy_budget)
            temp_position += before - buy_budget
            if temp_position >= -short_cap + 8:
                break
    ...
    buy_sizes = [bid_size, max(3, bid_size - 4)]
    sell_sizes = [ask_size, max(1, ask_size)]
    if recycle_exit_push:
