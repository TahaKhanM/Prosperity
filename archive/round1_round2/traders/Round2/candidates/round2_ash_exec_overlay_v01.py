"""Round 2 candidate: baseline Pepper plus narrow Ash medium-spread taker suppression."""
...
def trade_ash(
    ...
    medium_buy_spread_guard = 9 <= live_spread <= 10 and progress >= 0.12
    medium_sell_spread_guard = 10 <= live_spread <= 11 and progress >= 0.12
    if medium_buy_spread_guard:
        buy_take_edge += 1.15
        if weak_buy_support or cooldown_active:
            buy_take_edge += 0.45
    if medium_sell_spread_guard:
        sell_take_edge += 1.10
        if signal_bias >= 0.8:
            sell_take_edge += 0.35
