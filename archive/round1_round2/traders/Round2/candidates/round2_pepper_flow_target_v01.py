"""Round 2 candidate: baseline Pepper shell with persistent flow-conditioned targets."""
...
def trade_pepper(
    ...
    buyflow_streak = int(pepper_state.get("buyflow_streak", 0))
    sellflow_streak = int(pepper_state.get("sellflow_streak", 0))
    if trade_ema >= 4.0:
        buyflow_streak += 1
    else:
        buyflow_streak = max(0, buyflow_streak - 1)
    if trade_ema <= -4.0:
        sellflow_streak += 1
    else:
        sellflow_streak = max(0, sellflow_streak - 1)
    pepper_state["buyflow_streak"] = buyflow_streak
    pepper_state["sellflow_streak"] = sellflow_streak
    ...
    # Flow should move target inventory materially, not just quote skew.
    if buyflow_streak >= 2 and residual < 5.5 and regime_progress <= 0.92:
        target_long = min(long_cap, max(target_long, 80 if residual < 4.0 else 78))
        buy_edge = max(0.75, buy_edge - 0.20)
        bid_edge = max(0.9, bid_edge - 0.15)
        bid_size += 2
        if buyflow_streak >= 4:
            sweep_pad += 0.8
            sell_edge += 0.6
    if sellflow_streak >= 2 and position >= 74 and regime_progress >= 0.16:
        target_long = max(68, target_long - (4 if residual >= 4.0 else 2))
        sell_edge = max(3.2, sell_edge - 0.55)
        ask_edge = max(4.4, ask_edge - 0.45)
        ask_size += 2
        if sellflow_streak >= 4 and position >= 78:
            target_long = min(target_long, 68 if residual >= 4.0 else 70)
            bid_size = max(5, bid_size - 4)
            buy_edge += 0.25
    ...
    pepper_state["diag_gate_mask"] = (
        ...
        | (128 if buyflow_streak >= 2 else 0)
        | (256 if sellflow_streak >= 2 else 0)
    )
