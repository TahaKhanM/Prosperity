"""Round 2 candidate: baseline Pepper shell with a mild core-plus-satellite recycle state."""
...
def trade_pepper(
    ...
    if mode == "opening":
        ...
        core_target = 70
        satellite_target = 8
    reacquire_until = int(pepper_state.get("satellite_reacquire_until", -1))
    ...
    recycle_satellite = (
        position >= core_target + satellite_target
        and residual >= 4.0
        and 0.12 <= regime_progress <= 0.92
        and not buyflow
    )
    reacquire_satellite = (
        timestamp <= reacquire_until
        and position <= core_target + 2
        and residual <= 1.4
        and (buyflow or diagnostics["recent_slope"] <= -0.6 or residual <= -0.8)
    )
    if recycle_satellite:
        target_long = min(target_long, core_target)
        sell_edge = max(3.6, sell_edge - 0.35)
        ask_edge = max(4.6, ask_edge - 0.45)
        ask_size = max(ask_size, 3)
        pepper_state["satellite_reacquire_until"] = timestamp + 2200
    elif mode in {"opening", "discount"} or buyflow or residual <= 1.4 or reacquire_satellite:
        target_long = max(target_long, min(long_cap, core_target + satellite_target))
        if reacquire_satellite:
            buy_edge = max(0.9, buy_edge - 0.20)
            sweep_pad += 0.45
            bid_edge = max(1.0, bid_edge - 0.15)
            bid_size += 2
    else:
        target_long = min(target_long, core_target)
    if timestamp > reacquire_until and not recycle_satellite:
        pepper_state["satellite_reacquire_until"] = -1
    ...
    pepper_state["diag_gate_mask"] = (
        ...
        | (32 if recycle_satellite else 0)
        | (128 if reacquire_satellite else 0)
