"""Round 2 candidate: baseline Pepper shell with an empty-book Pepper guard."""
...
def pepper_state_and_fair(
    ...
    book_gap = live_bid is None and live_ask is None
    ...
    if book_gap:
        pepper_state["gap_guard_until"] = max(
            int(pepper_state.get("gap_guard_until", -1)),
            timestamp + PEPPER_GAP_GUARD_TICKS,
        )
    ...
    diagnostics = {
        ...
        "book_gap": 1.0 if book_gap else 0.0,
    }
...
def trade_pepper(
    ...
    gap_guard_until = int(pepper_state.get("gap_guard_until", -1))
    ...
    book_gap = bool(diagnostics.get("book_gap", 0.0))
    gap_guard_active = book_gap or timestamp <= gap_guard_until

    if gap_guard_active:
        buy_edge += 0.85
        sweep_pad = min(sweep_pad, 1.8)
        bid_edge += 0.65
        bid_size = max(6, bid_size - 5)
        if position >= 72:
            target_long = min(target_long, 74 if regime_progress < 0.33 else 72)
            sell_edge = max(3.4, sell_edge - 0.6)
            ask_edge = max(4.2, ask_edge - 0.7)
    ...
    pepper_state["diag_gate_mask"] = (
        ...
        | (128 if gap_guard_active else 0)
    )
