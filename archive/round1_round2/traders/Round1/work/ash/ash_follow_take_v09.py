...
def trade_ash(
    ...
    wide_buy_skew_signal = (
        spread >= 15
        and top_imbalance >= 0.30
        and micro_gap >= 0.45
        and (
            current_mid <= ASH_ANCHOR - 1.5
            or fair <= ASH_ANCHOR - 1.25
            or ret1 <= -1.0
        )
    )
    wide_sell_skew_signal = (
        spread >= 15
        and top_imbalance <= -0.30
        and micro_gap <= -0.45
        and (
    ...
    buy_follow_take_signal = (
        ...
        and not (progress >= 0.88 and temp_position >= 24)
        and (
            spread <= 10
    ...
    def allow_sell_clear_at(price: int) -> bool:
        ...
        if medium_sell_spread:
            if (
                0 < top_bid_volume <= 5
                and current_mid >= ASH_ANCHOR + 5.0
                and temp_position < 72
                and (
                    not shock
                    or (
                        temp_position <= 62
                        and top_imbalance > -0.40
                        and micro_gap > -2.40
                    )
                )
            ):
    ...
    wide_buy_skew = (
        wide_buy_skew_signal
        and not one_sided_sell_follow
        and temp_position < max(28, soft_cap + 10)
        and not (progress >= 0.88 and temp_position >= 18)
    )
    wide_sell_skew = (
        wide_sell_skew_signal
        and not one_sided_buy_follow
        and temp_position > -max(32, soft_cap + 12)
        and not (progress >= 0.88 and temp_position <= -18)
    )
    ...
    if one_sided_buy_follow:
        ...
        quote_fair = max(quote_fair, current_mid + 0.12)
    elif wide_sell_skew:
        quote_fair = min(quote_fair, current_mid - 0.12)
    ...
    elif wide_buy_skew and bid is not None and ask is not None and bid + 1 < ask:
        base_bid = max(base_bid, bid + 1)
    ...
    elif wide_sell_skew and bid is not None and ask is not None and ask - 1 > bid:
        base_ask = min(base_ask, ask - 1)
    ...
    if wide_buy_skew:
        buy_sizes[0] += 3
        sell_sizes[0] = max(6, sell_sizes[0] - 2)
    elif wide_sell_skew:
        sell_sizes[0] += 3
        buy_sizes[0] = max(6, buy_sizes[0] - 2)
    ...
    if progress >= 0.88 and position > 24:
        buy_sizes = [max(0, buy_sizes[0] - 4), max(0, buy_sizes[1] - 2)]
        sell_sizes = [sell_sizes[0] + 3, sell_sizes[1] + 2]
        base_ask -= 1
    elif progress >= 0.88 and position < -24:
        buy_sizes = [buy_sizes[0] + 3, buy_sizes[1] + 2]
        sell_sizes = [max(0, sell_sizes[0] - 4), max(0, sell_sizes[1] - 2)]
        base_bid += 1
