...
def trade_ash(
    ...
    shock_flat_buy_partial_take = (
        buy_follow_signal
        and spread == 10
        and shock
        and top_imbalance >= 0.42
        and micro_gap >= 2.0
        and diag["ret1"] <= -3.0
    ...
    did_shock_buy_partial_take = False
    if best_ask_level is not None and shock_flat_buy_partial_take:
        before = buy_budget
        buy_budget = add_buy(
            orders,
            ASH,
            best_ask_level,
            min(6, -depth.sell_orders[best_ask_level]),
            buy_budget,
        )
        temp_position += before - buy_budget
        did_shock_buy_partial_take = before != buy_budget

    if (
        best_ask_level is not None
        and not did_shock_buy_partial_take
        and allow_buy_take_at(best_ask_level)
    ):
        before = buy_budget
        buy_budget = add_buy(
            orders,
