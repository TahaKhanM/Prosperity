...
def classify_state(ctx: Dict[str, float], position: int) -> str:
    extreme_cheap = ctx["anchor_dev"] <= -4.0
    extreme_rich = ctx["anchor_dev"] >= 4.0
    buy_aligned = ctx["pressure"] >= 0.22 or ctx["buy_run"] >= 2 or ctx["up_run"] >= 2
    sell_aligned = ctx["pressure"] <= -0.22 or ctx["sell_run"] >= 2 or ctx["down_run"] >= 2

    if ctx["ask_only"] and position > -70:
        return "ask_only_continue_sell"
    if ctx["bid_only"] and position < 70:
        return "bid_only_continue_buy"
    if (extreme_cheap and buy_aligned) or (ctx["anchor_dev"] <= -2.0 and ctx["pressure"] >= 0.32):
        return "cheap_follow_buy"
    if (extreme_rich and sell_aligned) or (ctx["anchor_dev"] >= 2.0 and ctx["pressure"] <= -0.32):
        return "rich_follow_sell"
    if position >= 52 and ctx["pressure"] > -0.25 and not ctx["ask_only"]:
        return "recycle_long"
    if position <= -52 and ctx["pressure"] < 0.25 and not ctx["bid_only"]:
        return "recycle_short"
    if abs(ctx["anchor_dev"]) < 1.4 and abs(ctx["pressure"]) < 0.18 and ctx["spread"] < 99:
        return "neutral_make"
    return "stand_down"
...
def trade_ash(
    ...
    orders: List[Order] = []
    state = classify_state(ctx, position)

    if state == "neutral_make":
    ...
    elif state == "bid_only_continue_buy":
        buy_budget = post_passive_buy(
            orders,
            depth,
            fair + 0.35,
            buy_budget,
            size_for_side(18, position, "buy"),
            min_edge=0.7,
            promote=True,
        )
        buy_budget = post_passive_buy(
            ...
            fair - 0.55,
            buy_budget,
            size_for_side(8, position, "buy"),
            min_edge=1.0,
            promote=False,
        )
    elif state == "ask_only_continue_sell":
        sell_budget = post_passive_sell(
            orders,
            depth,
            fair - 0.35,
            sell_budget,
            size_for_side(18, position, "sell"),
            min_edge=0.7,
            promote=True,
        )
        sell_budget = post_passive_sell(
            ...
            fair + 0.55,
            sell_budget,
            size_for_side(8, position, "sell"),
            min_edge=1.0,
            promote=False,
        )
    elif state == "cheap_follow_buy":
        if ctx["anchor_dev"] <= -5.0 and (ctx["buy_run"] >= 2 or ctx["pressure"] >= 0.55) and position < 28:
    ...
    elif state == "rich_follow_sell":
        if ctx["anchor_dev"] >= 5.0 and (ctx["sell_run"] >= 2 or ctx["pressure"] <= -0.55) and position > -28:
    ...
    elif state == "recycle_long":
        if position >= 68 and ctx["pressure"] > -0.12:
            sell_budget, sold = post_cross_sell(orders, depth, fair, sell_budget, 8, edge=0.75)
            position -= sold
        sell_budget = post_passive_sell(
            orders,
            depth,
    ...
    elif state == "recycle_short":
        if position <= -68 and ctx["pressure"] < 0.12:
            buy_budget, bought = post_cross_buy(orders, depth, fair, buy_budget, 8, edge=0.75)
            position += bought
        buy_budget = post_passive_buy(
            orders,
            depth,
