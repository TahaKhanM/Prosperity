"""Build a per-tick voucher panel for Round 3.

Input
-----
Round 3 `prices_round_3_day_{0,1,2}.csv` under a configurable dataset root.
...
-----
    python3 build_voucher_panel.py \
        ...
        --out-dir ../../../prosperity-research/03_eda/round3 \
...
# TTE at start of each historical day (see README.md).
...
def read_prices_csv(path: Path) -> Dict[Tuple[int, int, str], float]:
    """Return {(day, timestamp, product): mid}."""
...
def load_day(dataset_root: Path, day: int) -> Dict[Tuple[int, str], float]:
    p = dataset_root / f"prices_round_3_day_{day}.csv"
    raw = read_prices_csv(p)
    # collapse to (timestamp, product) -> mid
...
def build_panel(
    ...
    for day in sorted(TTE_BY_DAY):
        ...
        for ts in timestamps:
            ...
            if s is None:
                continue
            # linearly shrink TTE within the day: at ts=0 use day_tte_start,
            # at the end of the day use day_tte_start - 1.
            progress = (ts // (10_000 // max(ticks_per_day, 1))) / max(
                ticks_per_day, 1
            ) if ticks_per_day else 0.0
            # Guard: Prosperity timestamps increment by 100. 10_000 // 100 = 100
            # ticks in a day, each 100 ts units wide.
...
def main() -> int:
    ...
    default_out = here.parents[2] / "prosperity-research" / "03_eda" / "round3"
    ...
    p.add_argument(
        ...
        type=float,
        default=365.0,
        help="Days per year (e.g. 365, 252, or 7 for 'round units in weeks').",
    )
