"""Identity-only arbitrage scanner for the Round 4 voucher chain.

Same four checks as the Round 3 version. The only thing that changes is the
default panel path (Round 4 panel) and the default output path. Voucher
strikes and ladder identities are unchanged.

Identities checked:

1. Intrinsic floor:        V_K >= max(S - K, 0)
2. Upper bound:            V_K <= S
3. Strike monotonicity:    K1 < K2  =>  V_{K1} >= V_{K2}
4. Convexity (butterfly):  V_{K1} - 2 V_{K2} + V_{K3} >= 0  for evenly spaced K

Usage
-----
    python3 parity_scan.py \
        --panel ../../../prosperity-research/03_eda/round4/voucher_panel.csv \
        --out ../../../prosperity-research/04_signal_notes/round4/parity_violations.csv
"""
...
def scan_intrinsic_and_upper(row: Dict[str, str], s: float) -> List[Dict[str, object]]:
    ...
    for k in STRIKES:
        ...
        if v < intrinsic - 1e-6:
            out.append({
                ...
                "strike_a": k, "strike_b": "", "strike_c": "",
                "lhs": v, "rhs": intrinsic, "edge": intrinsic - v,
            })
        if v > s + 1e-6:
            out.append({
                "check": "upper_bound_S",
                "strike_a": k, "strike_b": "", "strike_c": "",
                "lhs": v, "rhs": s, "edge": v - s,
            })
    return out
    ...
    for i in range(len(strikes_sorted) - 1):
        ...
        if v1 < v2 - 1e-6:
            out.append({
                ...
                "strike_a": k1, "strike_b": k2, "strike_c": "",
                "lhs": v1, "rhs": v2, "edge": v2 - v1,
            })
    return out
    ...
    for i in range(len(strikes_sorted) - 2):
        ...
        if fly < -1e-6:
            out.append({
                ...
                "strike_a": k1, "strike_b": k2, "strike_c": k3,
                "lhs": fly, "rhs": 0.0, "edge": -fly,
            })
    return out
...
def main() -> int:
    ...
    default_out = (
        here.parents[2]
        / "prosperity-research" / "04_signal_notes" / "round4" / "parity_violations.csv"
    )
