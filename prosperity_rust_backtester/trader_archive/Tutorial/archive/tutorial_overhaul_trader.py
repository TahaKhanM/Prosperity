...
TOMATO_RET1_WEIGHT = -0.6
TOMATO_Z20_WEIGHT = -0.3
TOMATO_TAKE_EDGE = 2.5
TOMATO_PASSIVE_EDGE = 1.5
TOMATO_PASSIVE_SIZE = 5
TOMATO_INV_SKEW = 0.01
TOMATO_SOFT_CAP = 35
TOMATO_REDUCE_EDGE = 0.75
TOMATO_CAP_EXTRA_EDGE = 2.0
...
def trade_tomatoes(
    ...
    for ask in sorted(depth.sell_orders):
        ...
        if edge < TOMATO_TAKE_EDGE:
            break
        if position < TOMATO_SOFT_CAP or edge >= TOMATO_TAKE_EDGE + TOMATO_CAP_EXTRA_EDGE:
            buy_budget = add_buy(
                orders, "TOMATOES", ask, -depth.sell_orders[ask], buy_budget
            )
    ...
    for bid in sorted(depth.buy_orders, reverse=True):
        ...
        if position > -TOMATO_SOFT_CAP or edge >= TOMATO_TAKE_EDGE + TOMATO_CAP_EXTRA_EDGE:
            sell_budget = add_sell(
                orders, "TOMATOES", bid, depth.buy_orders[bid], sell_budget
            )
    ...
    buy_edge_needed = TOMATO_REDUCE_EDGE if position <= -TOMATO_SOFT_CAP else TOMATO_PASSIVE_EDGE
    sell_edge_needed = TOMATO_REDUCE_EDGE if position >= TOMATO_SOFT_CAP else TOMATO_PASSIVE_EDGE

    if (
        position < TOMATO_SOFT_CAP
        and passive_bid < ask
        and fair - passive_bid >= buy_edge_needed
    ):
        buy_budget = add_buy(
    ...
    if (
        position > -TOMATO_SOFT_CAP
        and passive_ask > bid
        and passive_ask - fair >= sell_edge_needed
    ):
        sell_budget = add_sell(
