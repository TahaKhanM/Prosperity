...
TOMATO_HISTORY = 25
TOMATO_SIGNAL_WINDOW = 60
TOMATO_MICRO_WEIGHT = 0.8
TOMATO_RET1_WEIGHT = -0.1
TOMATO_Z20_WEIGHT = -0.8
TOMATO_TREND_WEIGHT = 0.4
...
def tomato_base_fair(depth: OrderDepth, recent_mids: List[float]) -> Tuple[float, float]:
    ...
    fast_window = recent_mids[-5:] + [current_mid]
    trend = sum(fast_window) / len(fast_window) - sum(window) / len(window)
    mp = microprice(depth)
