...
simple_mid = base.simple_mid
microprice = base.microprice
wall_mid = base.wall_mid
mm10_mid = base.mm10_mid
...
ASH_SIGNAL_SCALE = env_float("ASH_SIGNAL_SCALE", 0.9)
ASH_SIGNAL_THRESHOLD = env_float("ASH_SIGNAL_THRESHOLD", 0.95)
ASH_TAKE_TILT = env_float("ASH_TAKE_TILT", 0.24)
ASH_OPPOSITE_TAKE_PENALTY = env_float("ASH_OPPOSITE_TAKE_PENALTY", 0.08)


def ash_fair_and_state(depth, ash_state: Dict[str, float]):
    fair, ash_state, diagnostics = base.ash_fair_and_state(depth, ash_state)
    current_mid = float(diagnostics["mid"])
    bid = best_bid(depth)
    ask = best_ask(depth)
    ...
    if bid is not None and ask is not None:
        ...
        if total > 0:
            top_imbalance = (bid_volume - ask_volume) / total

    current_micro = microprice(depth) or current_mid
    book_mid = mm10_mid(depth) or wall_mid(depth) or current_mid
    raw_signal = (
        0.50 * (current_micro - current_mid)
        + 4.20 * top_imbalance
        + 0.22 * (book_mid - current_mid)
    )
    signal = clamp(raw_signal * ASH_SIGNAL_SCALE, -2.0, 2.0)

    diagnostics["spread"] = float(spread)
    diagnostics["top_imbalance"] = top_imbalance
...
def trade_ash(depth, position: int, ash_state: Dict[str, float], timestamp: int):
    ...
    spread = int(diag.get("spread", 0))
    signal = float(diag.get("signal", 0.0))
    ...
    strong_pos_signal = signal >= ASH_SIGNAL_THRESHOLD
    strong_neg_signal = signal <= -ASH_SIGNAL_THRESHOLD
    buy_take_edge = take_edge
    sell_take_edge = take_edge
    if strong_pos_signal:
        buy_take_edge = max(1.0, buy_take_edge - ASH_TAKE_TILT)
        sell_take_edge += ASH_OPPOSITE_TAKE_PENALTY
    elif strong_neg_signal:
        sell_take_edge = max(1.0, sell_take_edge - ASH_TAKE_TILT)
        buy_take_edge += ASH_OPPOSITE_TAKE_PENALTY
    ...
    if best_ask_level is not None and best_ask_level <= fair - buy_take_edge:
        before = buy_budget
        buy_budget = add_buy(
            orders,
            ASH,
    ...
    if best_bid_level is not None and best_bid_level >= fair + sell_take_edge:
        before = sell_budget
