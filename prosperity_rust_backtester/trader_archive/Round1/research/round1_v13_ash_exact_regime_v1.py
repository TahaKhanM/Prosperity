...
def ash_top_imbalance(depth) -> float:
    bid = best_bid(depth)
    ask = best_ask(depth)
...
def ash_exact_fair_and_state(depth, ash_state: Dict[str, float]):
    ...
    diagnostics["book_mid"] = book_mid
    diagnostics["anchor_dev"] = clamp(book_mid - base.ASH_ANCHOR, -8.0, 8.0)
    diagnostics["top_imbalance"] = ash_top_imbalance(depth)
    return fair, ash_state, diagnostics


def trade_ash(depth, position: int, ash_state: Dict[str, float], timestamp: int):
    fair, ash_state, diag = ash_exact_fair_and_state(depth, ash_state)
    current_mid = float(diag["mid"])
    shock = bool(diag["shock"])
    top_imbalance = float(diag["top_imbalance"])
    anchor_dev = float(diag["anchor_dev"])
    ...
    buy_take_allowed = (
        ...
        and (
            (
                spread <= 12
                and anchor_dev <= -6.0
                and top_imbalance >= 0.18
                and ret1 <= -2.0
            )
            or (
                spread <= 8
                and anchor_dev <= -3.0
                and top_imbalance >= 0.0
                and ret1 <= -1.5
            )
        )
    )
    sell_take_allowed = (
        ...
        and (
            ...
            or (
                spread <= 8
                and anchor_dev >= 3.0
                and top_imbalance <= 0.05
                and ret1 >= 1.5
            )
        )
    )

    if buy_take_allowed and ask <= fair - take_edge:
        before = buy_budget
        buy_budget = add_buy(
            orders,
            ASH,
    ...
    if sell_take_allowed and bid >= fair + take_edge:
        before = sell_budget
    ...
    seen = set()
    for idx, (price, size) in enumerate(list(zip(buy_prices, buy_sizes))[:1]):
    ...
    seen.clear()
    for idx, (price, size) in enumerate(list(zip(sell_prices, sell_sizes))[:1]):
