...
def ash_fair_and_state(
    ...
    bid = best_bid(depth)
    ask = best_ask(depth)
    bid_volume = float(depth.buy_orders.get(bid, 0)) if bid is not None else 0.0
    ask_volume = float(-depth.sell_orders.get(ask, 0)) if ask is not None else 0.0
    total_top_volume = bid_volume + ask_volume
    imbalance = (
        (bid_volume - ask_volume) / total_top_volume if total_top_volume > 0 else 0.0
    )
    ...
    imbalance_bias = clamp(imbalance * 2.2, -1.4, 1.4)
    ...
    diagnostics = {
        ...
        "ret1": ret1,
        "imbalance": imbalance,
        "spread": float((ask - bid) if bid is not None and ask is not None else 16),
...
def trade_ash(
    ...
    if anchor_gap >= 7.0:
        take_edge += 0.5
    take_buy_edge = take_edge
    take_sell_edge = take_edge
    if imbalance >= 0.25:
        take_buy_edge = max(1.4, take_buy_edge - 0.35)
        take_sell_edge += 0.45
    elif imbalance <= -0.25:
        take_buy_edge += 0.20
        take_sell_edge = max(1.4, take_sell_edge - 0.25)
    clear_edge = 0.5
    ...
    prefer_inside_buy = spread >= 15 and not shock and imbalance >= -0.2
    prefer_inside_sell = spread >= 15 and not shock and imbalance <= 0.2
    ...
    if imbalance >= 0.30:
        buy_sizes = [buy_sizes[0] + 2, buy_sizes[1] + 1]
        sell_sizes = [max(6, sell_sizes[0] - 2), max(6, sell_sizes[1] - 1)]
    elif imbalance <= -0.30:
        buy_sizes = [max(6, buy_sizes[0] - 2), max(6, buy_sizes[1] - 1)]
        sell_sizes = [sell_sizes[0] + 2, sell_sizes[1] + 1]
    ...
    use_second_layer = spread >= 18 or abs(position) >= 42 or abs(imbalance) >= 0.45
