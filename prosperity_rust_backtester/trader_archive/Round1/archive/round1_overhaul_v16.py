...
def trade_ash(
    ...
    buy_prices = [
        math.floor(quote_fair - 3.8),
        math.floor(quote_fair - 5.8),
    ]
    sell_prices = [
        math.ceil(quote_fair + 3.8),
        math.ceil(quote_fair + 5.8),
    ]
...
def trade_pepper(
    ...
    book_fair = diagnostics["book_fair"]
    ...
    if recycle_mode:
        pepper_state["recent_recycle_ts"] = timestamp
    recent_recycle_ts = int(pepper_state.get("recent_recycle_ts", -10_000_000))
    recent_recycle = 0 <= timestamp - recent_recycle_ts <= 4_000
    reacquire_mode = (
        recent_recycle
        and position <= 75
        and 0.12 <= progress <= 0.88
        and residual <= 1.0
        and carry_left >= 140.0
        and not sellflow
    )
    ...
    if recycle_mode:
        ...
        target_long = min(long_cap, 78)
        buy_edge = max(0.9, buy_edge - 0.5)
        sell_edge += 1.0
        sweep_pad += 1.1
        bid_edge = max(1.2, bid_edge - 0.35)
        ask_edge += 0.8
        bid_size += 4
    ...
    if reacquire_mode:
        sweep_ceiling = max(sweep_ceiling, book_fair + 0.6)
    for ask in sorted(depth.sell_orders):
