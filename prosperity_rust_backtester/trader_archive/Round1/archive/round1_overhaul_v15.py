...
def ash_fair_and_state(
    ...
    bid = best_bid(depth)
    ask = best_ask(depth)
    bid_volume = depth.buy_orders[bid] if bid is not None else 0
    ask_volume = -depth.sell_orders[ask] if ask is not None else 0
    ...
    diagnostics = {
        ...
        "ret1": ret1,
        "imbalance": imbalance,
        "micro_bias": current_micro - current_mid,
        "spread": spread,
...
def trade_ash(
    ...
    imbalance = float(diag["imbalance"])
    micro_bias = float(diag["micro_bias"])
    spread = float(diag["spread"])
    safe_regime = (not shock) and 16.0 <= spread <= 19.0
    signal = clamp(2.8 * imbalance + 0.18 * micro_bias, -1.2, 1.2) if safe_regime else 0.0
    ...
    if anchor_gap >= 7.0:
        take_edge += 0.5
    take_buy_edge = max(1.5, take_edge - (0.2 if signal > 0.65 else 0.0))
    take_sell_edge = max(1.5, take_edge - (0.2 if signal < -0.65 else 0.0))
    clear_edge = 0.5
    ...
    bid = best_bid(depth)
    ask = best_ask(depth)
    quote_fair = fair - skew + 0.55 * signal
    bid_offset_1 = 3.8
    bid_offset_2 = 5.8
    ask_offset_1 = 3.8
    ask_offset_2 = 5.8
    if signal > 0.65:
        ...
        ask_offset_2 = 5.4
    buy_prices = [
        math.floor(quote_fair - bid_offset_1),
        math.floor(quote_fair - bid_offset_2),
    ]
    sell_prices = [
        math.ceil(quote_fair + ask_offset_1),
        math.ceil(quote_fair + ask_offset_2),
    ]
    buy_sizes = [10, 12] if not shock else [8, 10]
    sell_sizes = [10, 12] if not shock else [8, 10]
    if signal > 0.65:
        buy_sizes = [11, 13] if not shock else [9, 11]
        sell_sizes = [8, 10] if not shock else [7, 9]
    elif signal < -0.65:
        buy_sizes = [8, 10] if not shock else [7, 9]
        sell_sizes = [11, 13] if not shock else [9, 11]
