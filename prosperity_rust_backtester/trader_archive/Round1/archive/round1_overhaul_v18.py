...
def trade_ash(
    ...
    imbalance = float(diag.get("imbalance", 0.0))
    micro_bias = float(diag.get("micro_bias", 0.0))
    ...
    if anchor_gap >= 7.0:
        take_edge += 0.5
    signal = clamp(2.2 * imbalance + 0.22 * micro_bias, -1.2, 1.2)
    buy_take_edge = take_edge
    sell_take_edge = take_edge
    if not shock:
        if signal >= 0.25:
            buy_take_edge = max(1.4, buy_take_edge - 0.35)
            sell_take_edge += 0.2
        elif signal <= -0.25:
            sell_take_edge = max(1.4, sell_take_edge - 0.35)
            buy_take_edge += 0.2
    clear_edge = 0.5
    ...
    if bids_below_fair:
        ...
        if quote_fair - best_bid_below_fair <= join_edge:
    ...
    if asks_above_fair:
        ...
        if best_ask_above_fair - quote_fair <= join_edge:
    ...
    if spread >= 19:
        buy_sizes = [12, 14] if not shock else [9, 11]
        sell_sizes = [12, 14] if not shock else [9, 11]
    elif spread >= 18:
