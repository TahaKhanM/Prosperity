...
def ash_fair_and_state(
    ...
    bid_volume = depth.buy_orders[top_bid] if top_bid is not None else 0
    ask_volume = -depth.sell_orders[top_ask] if top_ask is not None else 0
    total_top = bid_volume + ask_volume
    imbalance = (bid_volume - ask_volume) / total_top if total_top > 0 else 0.0
    ...
    trade_ema = 0.78 * float(ash_state.get("trade_ema", 0.0)) + flow_now

    anchor_dev = clamp(book_mid - ASH_ANCHOR, -3.0, 3.0)
    micro_bias_raw = current_micro - current_mid
    micro_bias = clamp(micro_bias_raw, -1.8, 1.8)
    fade_bias = clamp(-0.45 * ret1, -1.5, 1.5)
    flow_bias = clamp(trade_ema / 7.0, -1.6, 1.6)
    signal = clamp(3.8 * imbalance + 0.65 * micro_bias_raw + 0.35 * flow_bias, -3.0, 3.0)
    ...
    shock = abs(current_mid - ASH_ANCHOR) >= 6.0 or abs(ret1) >= 3.5 or abs(signal) >= 2.1

    ash_state["last_mid"] = current_mid
    ash_state["last_fair"] = fair
    ash_state["trade_ema"] = trade_ema
    ash_state["prev_bid"] = top_bid
    ash_state["prev_ask"] = top_ask

    diagnostics = {
        "mid": current_mid,
        "ret1": ret1,
        "imbalance": imbalance,
        "micro_bias": micro_bias_raw,
        "signal": signal,
        "trade_ema": trade_ema,
...
def trade_ash(
    ...
    fair, ash_state, diag = ash_fair_and_state(depth, ash_state, market_trades)
    current_mid = diag["mid"]
    shock = bool(diag["shock"])
    signal = float(diag["signal"])
    trade_ema = float(diag["trade_ema"])
    buy_signal = signal >= 0.9
    sell_signal = signal <= -0.9
    strong_buy = signal >= 1.8
    strong_sell = signal <= -1.8
    ...
    take_buy_edge = 3.3 if not shock else 2.8
    take_sell_edge = 3.3 if not shock else 2.8
    if buy_signal:
        take_buy_edge -= 1.0
        take_sell_edge += 0.7
    if sell_signal:
    ...
    if strong_sell:
        take_sell_edge -= 0.35
    take_buy_edge = max(1.4, take_buy_edge)
    take_sell_edge = max(1.4, take_sell_edge)
    clear_edge = 0.5
    skew = clamp(position * 0.05, -2.6, 2.6)
    ...
    bid = best_bid(depth)
    ask = best_ask(depth)
    quote_fair = fair + 0.55 * signal - skew
    bid_offsets = [4.2, 6.0]
    ask_offsets = [4.2, 6.0]
    ...
    if buy_signal:
        bid_offsets = [3.8, 5.5]
        ask_offsets = [4.9, 6.8]
        buy_sizes = [10, 12] if not shock else [8, 10]
        sell_sizes = [6, 8] if not shock else [5, 7]
    elif sell_signal:
        bid_offsets = [4.9, 6.8]
        ask_offsets = [3.8, 5.5]
        buy_sizes = [6, 8] if not shock else [5, 7]
        sell_sizes = [10, 12] if not shock else [8, 10]

    if abs(trade_ema) >= 8.0:
        if trade_ema > 0:
            bid_offsets[0] = max(3.5, bid_offsets[0] - 0.2)
            ask_offsets[0] += 0.3
        else:
            ask_offsets[0] = max(3.5, ask_offsets[0] - 0.2)
            bid_offsets[0] += 0.3

    buy_prices = [
        math.floor(quote_fair - bid_offsets[0]),
        math.floor(quote_fair - bid_offsets[1]),
    ]
    sell_prices = [
        math.ceil(quote_fair + ask_offsets[0]),
        math.ceil(quote_fair + ask_offsets[1]),
    ]
    ...
    for idx, (price, size) in enumerate(zip(buy_prices, buy_sizes)):
        ...
        if ask is not None and price >= ask:
            price = ask - 1
        if price in seen or (ask is not None and price >= ask) or quote_fair - price < 0.9:
            continue
        seen.add(price)
    ...
    for idx, (price, size) in enumerate(zip(sell_prices, sell_sizes)):
        ...
        if bid is not None and price <= bid:
            price = bid + 1
        if price in seen or (bid is not None and price <= bid) or price - quote_fair < 0.9:
            continue
        seen.add(price)
    ...
    def run(self, state: TradingState):
        ...
        if ASH in state.order_depths:
            result[ASH], ash_state = trade_ash(
                ...
                ash_state,
                list(state.market_trades.get(ASH, [])),
            )
