...
TOMATO_MICRO_WEIGHT = 0.6
TOMATO_RET1_WEIGHT = -0.3
TOMATO_Z20_WEIGHT = -0.6
...
TOMATO_LATE_HOLD_CUT = 140000
TOMATO_LATE_HOLD_POS = 40
TOMATO_LATE_FAIR_GATE = 1.0
TOMATO_LATE_FAIR_BIAS = 3.0
...
def tomato_base_fair(depth: OrderDepth, recent_mids: List[float]) -> Tuple[float, float]:
    ...
    fair = current_mid + TOMATO_MICRO_WEIGHT * micro_delta + TOMATO_RET1_WEIGHT * ret1 + TOMATO_Z20_WEIGHT * z20
    return fair, current_mid
...
def trade_tomatoes(depth: OrderDepth, position: int, recent_mids: List[float], timestamp: int) -> List[Order]:
    orders: List[Order] = []
    buy_budget = LIMITS["TOMATOES"] - position
    sell_budget = LIMITS["TOMATOES"] + position
    base_fair, current_mid = tomato_base_fair(depth, recent_mids)
    late_hold = (
        timestamp >= TOMATO_LATE_HOLD_CUT
        and position < TOMATO_LATE_HOLD_POS
        and base_fair - current_mid >= TOMATO_LATE_FAIR_GATE
    )
    fair = base_fair + (TOMATO_LATE_FAIR_BIAS if late_hold else 0.0)
    ...
    if temp_position < TOMATO_SOFT_CAP and passive_bid < ask and fair - passive_bid >= TOMATO_PASSIVE_EDGE:
        before = buy_budget
        buy_budget = add_buy(orders, "TOMATOES", passive_bid, TOMATO_PASSIVE_SIZE, buy_budget)
        temp_position += before - buy_budget
    if (not late_hold or temp_position >= TOMATO_LATE_HOLD_POS) and temp_position > -TOMATO_SOFT_CAP and passive_ask > bid and passive_ask - fair >= passive_edge(temp_position):
        before = sell_budget
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if "TOMATOES" in state.order_depths:
            ...
            result["TOMATOES"] = trade_tomatoes(depth, int(state.position.get("TOMATOES", 0)), mids, int(state.timestamp))
            current_mid = mid_price(depth)
