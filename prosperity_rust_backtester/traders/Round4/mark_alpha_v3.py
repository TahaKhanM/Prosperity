...
NAMED_VFE_CP_SIGNALS = {
    ("Mark 67", "Mark 49"): ("LONG", 8, 0.4, 0.4), 
    ("Mark 67", "Mark 22"): ("LONG", 8, 0.3, 0.3),
    ("Mark 55", "Mark 14"): ("LONG", 20, 0.2, 0.2),
    ("Mark 14", "Mark 55"): ("SHORT", 20, 0.2, 0.2),
}
...
def _named_vfe_cp_bias(named_cp_state: Dict, timestamp: int) -> Tuple[float, float]:
    ...
    for x in active:
        start = int(x.get("start", timestamp))
        horizon = max(1, int(x.get("horizon", 1)))
        age = max(0, timestamp - start)

        # Linear decay: full strength at signal, zero near expiry.
        weight = max(0.0, 1.0 - age / horizon)

        vfe_bias += weight * float(x.get("vfe_bias", 0.0))
        option_s_bias += weight * float(x.get("option_s_bias", 0.0))
...
class Trader:
    def run(self, state: TradingState):
        ...
        if vd is not None and VFE in fs:
            ...
            safe_named_vfe_bias = 0.0

            # Only apply named alpha when market is close to our normal fair.
            # This prevents chasing when the market has already moved away.
            base_fair = fs[VFE] + skew + _cp_signed_skew(cp_state, VFE) + vfe_option_bias

            if mid is not None and abs(mid - fs[VFE]) < 1.5:
                safe_named_vfe_bias = named_vfe_bias

            fair = base_fair + safe_named_vfe_bias
        ...
        if raw_ve_micro is not None:
            ...
            safe_named_option_s_bias = 0.0

            vfe_mid = _mid(vd) if vd is not None else None
            if vfe_mid is not None and VFE in fs and abs(vfe_mid - fs[VFE]) < 1.5:
                safe_named_option_s_bias = named_option_s_bias

            ve_micro_for_smile = raw_ve_micro + underlying_bias + safe_named_option_s_bias
        else:
