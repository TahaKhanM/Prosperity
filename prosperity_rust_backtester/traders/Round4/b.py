...
def trade_hyd(depth, position, hyd_state, timestamp):
    orders = []
    bid = best_bid(depth)
    ask = best_ask(depth)
    if bid is None or ask is None:
    ...
    # Buy only very cheap.
    if ask <= fair - 12:
        qty = min(-depth.sell_orders[ask], 20)
        buy_budget = add_buy(orders, HYD, ask, qty, buy_budget)

    # Sell only very expensive.
    if bid >= fair + 12:
        qty = min(depth.buy_orders[bid], 20)
        sell_budget = add_sell(orders, HYD, bid, qty, sell_budget)

    # Inventory exit: if already long, sell near/above anchor.
    if position > 0 and bid >= fair + 1:
        qty = min(position, depth.buy_orders[bid], 20)
        sell_budget = add_sell(orders, HYD, bid, qty, sell_budget)

    # Inventory exit: if already short, buy near/below anchor.
    if position < 0 and ask <= fair - 1:
        qty = min(-position, -depth.sell_orders[ask], 20)
        buy_budget = add_buy(orders, HYD, ask, qty, buy_budget)
