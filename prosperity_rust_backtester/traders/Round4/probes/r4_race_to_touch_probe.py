"""Phase 3 — race-to-the-touch (Flavor 2 from Mark 14 dossier K.2).

For HYDROGEL_PACK only: track an inferred Mark 14 inside-touch level per
side (decayed toward mid each tick), and quote one tick INSIDE Mark 14's
last known location. The dossier evidence:
  - Mark 14 quotes inside the displayed touch (median price - ask_pre = -16
    on HYD).
  - 100 % passive provider (0/496 HYD buys lifted).
  - Adverse selection +8.65/u h=100 in their direction.

Implementation:
- On each market_trade in HYD with `buyer == "Mark 14"`, record
  `last_mark14_bid = trade.price` (Mark 14 was the resting bidder).
- Symmetric for `seller == "Mark 14"` → `last_mark14_ask = trade.price`.
- Each tick, decay: `last_mark14_bid += (mid - last_mark14_bid) * DRIFT_RATE`.
- Quote: bid = min(default_bid, last_mark14_bid + 1);
         ask = max(default_ask, last_mark14_ask - 1).
- Inventory governor: |position_HYD| > 0.7 × 200 → revert to v15 quoting.
- Race-loss detector: if Mark 14 prints at a price BETWEEN our last quote
  and the prevailing mid, count consecutive losses; after 5 in a row,
  pause for 200 ticks.

Pre-registered (locked):
  RTT_DRIFT_RATE          = 0.05         # last_mark14 → mid per tick
  RTT_INVENTORY_GOVERN    = 0.7          # of position limit
  RTT_RACE_LOSS_THRESHOLD = 5
  RTT_RACE_TIMEOUT_TICKS  = 200
  RTT_PRODUCTS            = ["HYDROGEL_PACK"]   # HYD only initially

Equally-defensible: drift ∈ [0.02, 0.10]; threshold ∈ [3, 10]; timeout ∈
[100, 500].

Ship gate: 3-day Δ vs floor ≥ +1,500 AND HYD own_trades count INCREASES
by ≥ 10% AND HYD product Δ ≥ +1,500. If HYD own_trades DROPS → kill
(we're standing aside).

Falsifiers: live R4 day-1 mean |trade.price - last_mark14_bid| > 5 ticks;
or live race-loss rate > 50 %.
...
# Phase 3 — Mark 14 race-to-the-touch constants (HYD only)
RTT_DRIFT_RATE = 0.05
RTT_INVENTORY_GOVERN = 0.7
RTT_RACE_LOSS_THRESHOLD = 5
RTT_RACE_TIMEOUT_TICKS = 200
RTT_TS_PER_TICK = 100
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Phase 3 — race-to-the-touch state
        rtt_bid = prior.get("rtt_bid")          # last_mark14_bid (HYD)
        rtt_ask = prior.get("rtt_ask")          # last_mark14_ask (HYD)
        rtt_loss_count = int(prior.get("rtt_loss_count", 0))
        rtt_timeout_until = int(prior.get("rtt_timeout_until", 0))
        my_last_hyd_bid = prior.get("my_last_hyd_bid")
        my_last_hyd_ask = prior.get("my_last_hyd_ask")
        ...
        # ---------- Phase 3: process Mark 14 HYD prints ------------------
        # Update rtt_bid/ask from Mark 14's HYD trades; detect race losses.
        hyd_trades = (market_trades or {}).get(HYD, []) or []
        hyd_mid_for_rtt = _mid(hd) if hd is not None else None
        for tr in hyd_trades:
            buyer_n = (getattr(tr, "buyer", None) or "").strip()
            seller_n = (getattr(tr, "seller", None) or "").strip()
            pr = float(getattr(tr, "price", 0) or 0)
            if pr <= 0:
                continue
            if buyer_n == "Mark 14":
                # Mark 14 was the resting bidder; print level becomes their bid.
                rtt_bid = pr
                # Race-loss check: did Mark 14 out-bid us?
                if (
                    hyd_mid_for_rtt is not None
                    and my_last_hyd_bid is not None
                    and pr > my_last_hyd_bid
                    and pr < hyd_mid_for_rtt
                ):
                    rtt_loss_count += 1
                else:
                    rtt_loss_count = 0
            elif seller_n == "Mark 14":
                rtt_ask = pr
                if (
                    hyd_mid_for_rtt is not None
                    and my_last_hyd_ask is not None
                    and pr < my_last_hyd_ask
                    and pr > hyd_mid_for_rtt
                ):
                    rtt_loss_count += 1
                else:
                    rtt_loss_count = 0
        if rtt_loss_count >= RTT_RACE_LOSS_THRESHOLD:
            rtt_timeout_until = int(state.timestamp) + RTT_RACE_TIMEOUT_TICKS * RTT_TS_PER_TICK
            rtt_loss_count = 0

        # Decay rtt_bid/ask toward current mid.
        if hyd_mid_for_rtt is not None:
            if rtt_bid is not None:
                rtt_bid = rtt_bid + (hyd_mid_for_rtt - rtt_bid) * RTT_DRIFT_RATE
            if rtt_ask is not None:
                rtt_ask = rtt_ask + (hyd_mid_for_rtt - rtt_ask) * RTT_DRIFT_RATE
        ...
        if hd is not None and HYD in fs:
            ...
            # Phase 3 — race-to-the-touch override (HYD only).
            rtt_active = (
                int(state.timestamp) >= rtt_timeout_until
                and abs(pos) <= RTT_INVENTORY_GOVERN * CAP_H
            )
            if rtt_active:
                if rtt_bid is not None:
                    bp_rtt = int(math.floor(rtt_bid)) + 1
                    bp = min(bp, bp_rtt)
                if rtt_ask is not None:
                    sp_rtt = int(math.ceil(rtt_ask)) - 1
                    sp = max(sp, sp_rtt)
                if bid is not None: bp = min(bp, bid + 1)
                if ask is not None:
                    bp = min(bp, ask - 1); sp = max(sp, ask - 1)
            ...
            # Track our last submitted quote for race-loss detection next tick.
            my_last_hyd_bid = bp
            my_last_hyd_ask = sp
            if legs:
                out[HYD] = legs
        ...
        td_out = {
            ...
            "rtt_bid": round(rtt_bid, 4) if rtt_bid is not None else None,
            "rtt_ask": round(rtt_ask, 4) if rtt_ask is not None else None,
            "rtt_loss_count": rtt_loss_count,
            "rtt_timeout_until": rtt_timeout_until,
            "my_last_hyd_bid": my_last_hyd_bid,
            "my_last_hyd_ask": my_last_hyd_ask,
            "smile": {
