"""Phase 2.C — deadband fair-value-bias hedge on top of composite v01.

R4-CHAIN-01-V2 (corrected). Each tick, compute portfolio voucher delta
  D = sum_K [0.7 × Δ_BS(S, K, tte, sigma_smile_K)] × position_K
over K ∈ {4000..5500}. If |D| ≥ FV_BIAS_DEADBAND, add bias to VFE MM fair:
  bias_VFE = -D / FV_BIAS_DIVISOR, clipped to ±FV_BIAS_HARD_CAP.
Otherwise bias_VFE = 0. Apply as additive skew (NOT take order).

Different from the prior failed Phase 4 (`r4_chain_01_fvbias_probe.py`):
that probe used divisor=4 with no deadband; bias hit ±1 cap on most ticks
and VFE crashed −35,892. This version raises divisor to 8 and adds a
deadband at |D|=100.

Pre-registered (locked):
  FV_BIAS_DELTA_RATIO  = 0.70   # smile-Δ over-states empirical β by ~30%
  FV_BIAS_DEADBAND     = 100    # only bias when |D| > 100
  FV_BIAS_DIVISOR      = 8.0    # raised from prior 4.0
  FV_BIAS_HARD_CAP     = 1.0    # tick

Equally-defensible: deadband ∈ [50,200]; divisor ∈ [6,12]; cap ∈ [0.5,1.5].

Ship gate: 3-day Δ vs composite v01 ≥ +500 AND VFE own_trades drops by ≥5%
AND no single-day Δ < −500. Falsifier: VFE own_trades INCREASES by >10% →
the bias is wrong-signed (existing MM is fighting it). Kill.
...
# Phase 2.C — deadband FV-bias hedge constants
FV_BIAS_DELTA_RATIO = 0.70
FV_BIAS_DEADBAND = 100.0
FV_BIAS_DIVISOR = 8.0
FV_BIAS_HARD_CAP = 1.0
FV_BIAS_STRIKES = (4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500)
...
class Trader:
    def run(self, state: TradingState):
        ...
        # ---------- Phase 2.C — deadband FV-bias hedge --------------------
        # Compute portfolio voucher delta D from PRIOR tick's smile state +
        # CURRENT positions. First tick: prior smile state is empty → bias=0.
        fv_bias = 0.0
        sm_a0 = smile_state.get("smile_a0")
        sm_a1 = smile_state.get("smile_a1")
        sm_a2 = smile_state.get("smile_a2")
        sm_s = smile_state.get("smile_s")
        sm_tte_days = smile_state.get("smile_tte_days")
        if (sm_a0 is not None and sm_a1 is not None and sm_a2 is not None
                and sm_s is not None and sm_tte_days is not None
                and sm_s > 0 and sm_tte_days > 0):
            tte_yrs = float(sm_tte_days) / 365.0
            D_total = 0.0
            for kk in FV_BIAS_STRIKES:
                pos_kk = positions.get(f"VEV_{kk}", 0)
                if pos_kk == 0:
                    continue
                # smile-fit IV at this strike (clamp moneyness to a sane band
                # for deep-ITM extrapolation; Δ is dominated by moneyness).
                m_kk = math.log(float(sm_s) / float(kk))
                m_clamp = max(-0.20, min(0.20, m_kk))
                iv_kk = float(sm_a0) + float(sm_a1) * m_clamp + float(sm_a2) * m_clamp * m_clamp
                if iv_kk <= 0.005:
                    iv_kk = float(sm_a0)
                delta_kk = bs_call_delta(float(sm_s), float(kk), tte_yrs, iv_kk)
                D_total += FV_BIAS_DELTA_RATIO * delta_kk * float(pos_kk)
            if abs(D_total) >= FV_BIAS_DEADBAND:
                fv_bias = -D_total / FV_BIAS_DIVISOR
                if fv_bias > FV_BIAS_HARD_CAP: fv_bias = FV_BIAS_HARD_CAP
                if fv_bias < -FV_BIAS_HARD_CAP: fv_bias = -FV_BIAS_HARD_CAP
        ...
        if vd is not None and VFE in fs:
            ...
            fair = fs[VFE] + skew + vfe_fade + lean_v + fv_bias
            legs = []; b = s = 0
