"""Phase 1 followup: r4_phase5_g1 - per-strike rolling mid-IV (IV-domain).

Replaces v15's static `SMILE_PER_STRIKE_BIAS[K]` + parabolic-fit fair IV with
a per-strike rolling EMA of observed mid-IV.

Each tick, for each smile-list strike K:
  1. iv_m_K = implied_vol_call(V_K_mid, S=VFE_micro, K, T=tte_yr) -- already
     computed by v15.
  2. Maintain per-strike EMA: iv_ema_K = (1-alpha)*iv_ema_K_prev + alpha*iv_m_K
     with HL=60 ticks => alpha = 1 - exp(-ln(2)/60) ~ 0.01149.
  3. Use iv_ema_K (NOT a0+a1*m+a2*m^2+bias) as fair_iv -> bs_call_price.
  4. Seed iv_ema_K from current iv_m_K on first tick where iv_m is valid.
     Fall back to smile-fit + bias if iv_m invalid (deep-ITM solver returns 0).

Differs from Phase 8b's MIS-implementation (which blended micro/wall in price
domain at 50/50 with smile fair). This is the IV-domain rolling EMA the
Phase 5 past-winner sweep originally identified as the highest-EV gap.

Pre-reg:
- IV_EMA_HL_TICKS = 60 (HL ~6 game-seconds; range [30, 120] equally defensible).
- Seed per-strike from current observation (washes within ~5 HLs).
- Static SMILE_PER_STRIKE_BIAS dict KEPT only as a NaN fallback.

Ship gate: Δ vs v15 ≥ +1,500 AND day-3 ≥ +500 AND per-day Δ ≥ −500.
...
# R4-PHASE5-G1 per-strike rolling mid-IV constants
IV_EMA_HL_TICKS = 60
IV_EMA_ALPHA = 1.0 - math.exp(-math.log(2.0) / IV_EMA_HL_TICKS)  # ~0.01149
...
def _smile_voucher_orders(state, depths, positions, timestamp, ve_micro):
    ...
    iv_ema_state = state.setdefault("iv_ema", {})
    for (k, m, iv_m_obs, v) in strike_data:
        sym = f"VEV_{k}"
        d_ = depths.get(sym)
        if d_ is None:
            continue
        kstr = str(k)
        # R4-PHASE5-G1: per-strike rolling EMA of observed mid-IV.
        prev_ema = iv_ema_state.get(kstr)
        if prev_ema is None:
            # Seed with current observed iv (Newton-solver result for this tick).
            # Fallback: smile-fit + static bias if iv_m_obs is degenerate.
            if iv_m_obs > 0.005:
                seed_iv = iv_m_obs
            else:
                bias_k_seed = SMILE_PER_STRIKE_BIAS.get(k, 0.0)
                seed_iv = a0 + a1 * m + a2 * m * m + bias_k_seed
            seed_iv = max(seed_iv, 0.05)
            iv_ema_state[kstr] = seed_iv
            prev_ema = seed_iv
        # Update EMA toward current observed iv (skip if iv_m_obs degenerate).
        if iv_m_obs > 0.005:
            new_ema = (1.0 - IV_EMA_ALPHA) * prev_ema + IV_EMA_ALPHA * iv_m_obs
            iv_ema_state[kstr] = new_ema
            fair_iv = new_ema
        else:
            fair_iv = prev_ema  # hold last known EMA
        if fair_iv <= 0.005:
            continue
