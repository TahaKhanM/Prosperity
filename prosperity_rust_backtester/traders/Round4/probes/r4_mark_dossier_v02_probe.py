...
Built on top of `r4_mark_lean_v01_probe.py`. Locks all v01 settings except the
mark-lean module, which is replaced with the Phase-5 playbook rules from
`prosperity-research/04_signal_notes/round4/mark_exploitation_playbook.md`.

Key changes vs v01:
1. **Counterparty-conditional gating**: Mark 14 / Mark 38 / Mark 49 / Mark 55
   contributions are scaled by whether the counterparty matches the structural
   dyad partner (per Phase 3 cross-Mark coordination).
2. **Per-Mark decay**: Mark 67 uses a faster decay (HL ~20 ticks) because
   the dossier shows the alpha decays past h=20. Other Marks use HL ~50
   (vs v01's HL ~46).
3. **Larger Mark 14 / Mark 38 weights on HYD** (the highest-EV per-print
   alpha).
4. **Smaller Mark 55 weight** (tail-driven loss; less robust per-print
   capture).
...
# HYD: Mark 14 (smart bot) and Mark 38 (bag-holder) are mirrors. Both weight
# +0.40 (vs +0.20 in v01) — larger because the per-print alpha (~+8/u h=100)
# is the largest in the dataset.
HYD_W_M14 = 0.40
HYD_W_M38 = 0.40
HYD_W_M22_BUY_FADE = 0.30   # adversarial small-N fade

# VEV_4000: same dyad but smaller weight; the alpha is mostly delta-1 to VFE.
VEV4000_W_M14 = 0.20
VEV4000_W_M38 = 0.20

# VFE: multiple Marks contribute.
VFE_W_M14 = 0.15            # smart bot, both sides
VFE_W_M38 = 0.0             # Mark 38 doesn't trade VFE
VFE_W_M67 = 0.50            # informed buyer, short-horizon (uses fast decay)
VFE_W_M49_SELL = 0.25       # fade their sells
VFE_W_M22_SELL = 0.15       # fade their sells
VFE_W_M55 = 0.10            # fade their direction (light)
VFE_W_M01 = 0.10            # mirror their direction (low priority)

# Counterparty-conditional multipliers. When acting on a Mark print, scale
# the weight by the multiplier for the counterparty.
# {Mark X: {Mark Y (counterparty): multiplier}}
CP_MULT = {
    "Mark 14": {"Mark 38": 1.0, "Mark 22": 0.0, "_default_": 0.5},
    "Mark 38": {"Mark 14": 1.0, "Mark 22": 0.0, "_default_": 0.5},
    "Mark 49": {"Mark 67": 1.0, "Mark 55": 0.0, "_default_": 0.5},
    "Mark 55": {"Mark 14": 1.0, "Mark 01": 1.0, "_default_": 0.5},
    "Mark 22": {"_default_": 1.0},
    "Mark 67": {"_default_": 1.0},  # always copy, no gating
    "Mark 01": {"Mark 55": 1.0, "_default_": 0.5},
}
