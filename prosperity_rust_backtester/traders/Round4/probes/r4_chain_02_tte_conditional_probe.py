"""Phase 3 followup: r4_chain_02_tte_conditional - TTE-conditional smile a2.

REPLACES the v15/baseline `_smile_prior` linear formula
  a2(tte) = SMILE_A2_BASE + SMILE_A2_DRIFT_PER_DAY * (8 - tte)
with a per-TTE-day LOOKUP table (locked from term_structure_smile.csv) and
LINEAR INTERPOLATION between bucket points:

  TTE  7: a2 = 7.62   (FIT)
  TTE  6: a2 = 8.53   (FIT)
  TTE  5: a2 = 14.11  (FIT, 5σ spike)
  TTE  4: a2 = 16.58  (EXTRAP)
  TTE  3: a2 = 19.82  (EXTRAP)
  TTE >7 → clamp at 7.62
  TTE <3 → clamp at 19.82

This fixes the Phase 8b r4_smile_refit_probe failure where the constants-swap
form (SMILE_A2_BASE=7.62, SMILE_A2_DRIFT_PER_DAY=1.62) over-flattened TTE 7
because v15's linear extrapolation was already a2 ≈ 8.03 at TTE 7 (close to
the measured 7.62), and the drift-1.62 form gave a2 = 9.24 (overshoots).
The lookup table preserves the measured per-TTE fit at every bucket.

Pre-registered: lookup values fixed; linear-interpolation method picked
ex-ante to avoid discontinuities at half-day boundaries.
...
SMILE_A2_BASE = 7.21          # legacy v15 (unused; kept for diff clarity)
SMILE_A2_DRIFT_PER_DAY = 0.815  # legacy v15 (unused)

# R4-CHAIN-02 TTE-conditional convexity lookup (FIXED, do not tune)
SMILE_A2_BY_TTE: Dict[int, float] = {
    7: 7.62,   # FIT (R4 day 1)
    6: 8.53,   # FIT (R4 day 2)
    5: 14.11,  # FIT (R4 day 3) — 5σ spike vs TTE 6
    4: 16.58,  # EXTRAP (R4 live day 1)
    3: 19.82,  # EXTRAP (R4 live day 2)
}
SMILE_A2_TTE_LO = 3
SMILE_A2_TTE_HI = 7


def _smile_a2_lookup(tte_days: float) -> float:
    """Linear interpolation between integer-day bucket points; clamp at ends."""
    if tte_days >= SMILE_A2_TTE_HI:
        return SMILE_A2_BY_TTE[SMILE_A2_TTE_HI]
    if tte_days <= SMILE_A2_TTE_LO:
        return SMILE_A2_BY_TTE[SMILE_A2_TTE_LO]
    lo = int(math.floor(tte_days))
    hi = int(math.ceil(tte_days))
    if lo == hi:
        return SMILE_A2_BY_TTE[lo]
    f = tte_days - lo
    return SMILE_A2_BY_TTE[lo] * (1.0 - f) + SMILE_A2_BY_TTE[hi] * f
...
def _smile_prior(tte_days):
    ...
    # R4-CHAIN-02: a2 from per-TTE lookup table (linear interp), NOT linear drift.
    a2 = _smile_a2_lookup(tte_days)
    return a0, a1, a2
