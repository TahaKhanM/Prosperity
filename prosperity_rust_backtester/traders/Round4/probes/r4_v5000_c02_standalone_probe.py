"""Phase 2 followup: r4_v5000_c02_standalone - imb2 skew on V_5000 only.

Single delta over r4_baseline_v15: in `_smile_voucher_orders`, when k==5000,
add `+V5000_IMB_BETA * imb_k2(d_)` to the per-strike fair value, capped at
±V5000_IMB_CAP ticks. All other strikes unchanged.

Phase 1 sub-agent measurement: imb_k2 → next-tick Δmid β = +3.05/+3.59/+3.15
(t = +12.4/+15.1/+13.7) across days 1/2/3 — stable and large. β=+3 picked
from the median; cap at 1 tick (analogous to V_SKEW_CAP on VFE).

Phase 8b's Probe 7 stack reported V_5000 imb2 contributes +2,657 inside the
combined stack — but Probe 7 was contaminated by the V_5100 micro/wall
disaster. This probe ISOLATES V_5000 imb2 with NO other changes.

Pre-registered: β=3.0, cap=1.0. Range β∈[2,4], cap∈[0.5, 2.0].
Ship gate: Δ ≥ +1,500 AND V_5000 carries ≥80 % of the lift.
...
# R4-V5000-C02 standalone constants (NEW)
V5000_IMB_BETA = 3.0   # imb2 β median across days
V5000_IMB_CAP = 1.0    # cap +/- 1 tick (one tick of fair-value skew max)
...
def _smile_voucher_orders(state, depths, positions, timestamp, ve_micro):
    ...
    for (k, m, _, v) in strike_data:
        ...
        # R4-V5000-C02: imb_k2 -> fair skew on V_5000 only, cap +-1 tick
        if k == 5000:
