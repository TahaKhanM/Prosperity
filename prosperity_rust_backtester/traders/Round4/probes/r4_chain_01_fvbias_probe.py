"""Phase 4 followup: r4_chain_01_fvbias - 0.7xDelta hedge as fair-value bias.

REPLACEMENT for the Phase 8a `r4_combined_v01_probe` 0.7xDelta TAKE-order hedge
that blew up VFE by -60.9k. Same statistically-valid signal (sticky-strike
adjustment: BS Delta consistently overstates realised 100-tick beta by ~30 %
across all R4 days) but implemented as a FAIR-VALUE BIAS that the existing
VFE MM (wall-mid + AC1 fade) absorbs naturally without spread-crossing.

Mechanism each tick:
1. D = sum_K (Delta_BS(S, K, T, sigma_K) * pos_K * 0.70) across all 10 strikes
2. vfe_fv_bias = -D / VFE_BIAS_DIVISOR (negative because long voucher Delta
   = long underlying => bias VFE fair DOWN to encourage VFE MM to sell)
3. Cap at +/-BIAS_HARD_CAP ticks
4. Add to VFE MM fair INLINE (not persisted to fs[VFE] state)

Pre-reg:
- HEDGE_DELTA_MULT = 0.70 (locked from Phase 2A measurement: median |Delta-beta|=0.06-0.32
  with t=16-51 across 30 cells).
- VFE_BIAS_DIVISOR = 4.0 (max portfolio Delta ~140 VFE-equivalent, /4 = 35 ticks
  uncapped, hard cap at 1 tick).
- BIAS_HARD_CAP = 1.0 tick.
- Range divisor [3.0, 6.0]; cap [0.5, 2.0].

Ship gate: Delta vs v15 >= +500 AND VFE trade count drops >= 5% AND VFE PnL
improves (the lift should be in inventory savings, not extra trades).
...
# R4-CHAIN-01 fair-value-bias hedge constants (NEW)
HEDGE_DELTA_MULT = 0.70
VFE_BIAS_DIVISOR = 4.0
BIAS_HARD_CAP = 1.0
...
class Trader:
    def run(self, state: TradingState):
        ...
        if vd is not None and VFE in fs:
            ...
            # R4-CHAIN-01 FV-BIAS: compute portfolio voucher delta with sticky-strike
            # 0.70 multiplier; bias VFE fair so the existing MM absorbs the rebalance.
            vfe_fv_bias = 0.0
            ve_micro_for_hedge = _micro(vd)
            if ve_micro_for_hedge is not None and ve_micro_for_hedge > 0:
                t_yr_h = _tte_years(state.timestamp)
                if t_yr_h > 0:
                    tte_days_h = t_yr_h * 365.0
                    a0_p_h, a1_h, a2_h = _smile_prior(tte_days_h)
                    a0_h = smile_state.get("smile_a0")
                    if a0_h is None:
                        a0_h = a0_p_h
                    D = 0.0
                    for k_h in ALL_VOUCHER_STRIKES:
                        sym_h = f"VEV_{k_h}"
                        pos_k_h = positions.get(sym_h, 0)
                        if pos_k_h == 0:
                            continue
                        bias_k_h = SMILE_PER_STRIKE_BIAS.get(k_h, 0.0)
                        m_log_h = math.log(ve_micro_for_hedge / float(k_h))
                        iv_k_h = a0_h + a1_h * m_log_h + a2_h * m_log_h * m_log_h + bias_k_h
                        if iv_k_h <= 0.005:
                            iv_k_h = 0.05
                        delta_k_h = bs_call_delta(ve_micro_for_hedge, float(k_h), t_yr_h, iv_k_h)
                        D += delta_k_h * float(pos_k_h) * HEDGE_DELTA_MULT
                    vfe_fv_bias = -D / VFE_BIAS_DIVISOR
                    if vfe_fv_bias > BIAS_HARD_CAP: vfe_fv_bias = BIAS_HARD_CAP
                    if vfe_fv_bias < -BIAS_HARD_CAP: vfe_fv_bias = -BIAS_HARD_CAP

            fair = fs[VFE] + skew + vfe_fade + vfe_fv_bias
            legs = []; b = s = 0
