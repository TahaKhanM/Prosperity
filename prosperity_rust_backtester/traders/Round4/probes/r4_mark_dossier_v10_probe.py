    ...
    for product, trades in (market_trades or {}).items():
        for t in trades:
            ...
            for mark, side, partner in (
                ...
                # v10: VFE drift regime gate applied to ALL +VFE-direction
                # contributions (not just Mark 67). When the gate kills, we
                # don't add +VFE leans but DO add -VFE leans (they protect
                # on down days). Identify direction by sign of contribution
                # before the multiplier.
                if product == VFE:
                    raw_contribution_sign = sgn * w  # +ve = adds +VFE; -ve = adds -VFE
                    if raw_contribution_sign > 0:
                        # only suppress LONG-side leans on down regimes
