"""Probe 3: r4_xprod_vev5200 - vega-coupling spillover.

When Mark 14 buys or Mark 22 sells VEV_5200, lean LONG deeper-ITM strikes
(V_4000/V_4500/V_5000/V_5100) for ~5 ticks. LEAN_DECAY=0.95 -> HL ~14 ticks.

Per-strike xprod_lean accumulator:
  On Mark 14 buyer or Mark 22 seller of VEV_5200, qty Q:
    xprod_lean[4000] += 1.0 * Q * 0.10
    xprod_lean[4500] += 0.7 * Q * 0.10
    xprod_lean[5000] += 0.3 * Q * 0.10
    xprod_lean[5100] += 0.2 * Q * 0.10

Clamp xprod_lean to +/- 5.0 ticks per strike.

In smile-MM (_smile_voucher_orders), add xprod_lean[k] to fair per strike.
For deep-ITM accumulation block (K in {4000,4500}), add to intr reference too.

Persist xprod_lean in traderData.
...
# Probe 3 NEW: cross-product VEV_5200 -> deep/ITM lean
XPROD_LEAN_DECAY = 0.95   # per-tick (HL ~14 ticks; ~5-tick window)
XPROD_LEAN_CLAMP = 5.0    # ticks per strike
# Per-strike weight on (qty * 0.10) for Mark 14 buyer / Mark 22 seller of VEV_5200
XPROD_WEIGHTS = {
    4000: 1.0,
    4500: 0.7,
    5000: 0.3,
...
XPROD_TRIGGERS_BUYER = {"Mark 14"}  # buyer side trigger
XPROD_TRIGGERS_SELLER = {"Mark 22"}  # seller side trigger
...
class Trader:
    def run(self, state: TradingState):
        ...
        # Probe 3 NEW: update xprod lean from market trades
        ...
        # ---------- Phase 1 smile MM (v9 unchanged + xprod lean) ----------
