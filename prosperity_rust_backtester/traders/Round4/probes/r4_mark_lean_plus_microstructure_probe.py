"""Probe 7: r4_mark_lean_plus_microstructure - mark-lean + voucher microstructure.

Combines:
  - Probe 1: per-product Mark-conditional lean stack on HYD/VFE.
  - R4-V5000-C02: V_5000 imb_k2 -> fair skew (beta=+3.0, capped at 1.0 tick).
  - R4-V5100-C01: V_5100 microprice/wall blend at 50/50 with smile fair:
        fair_v5100 = (smile_fair + 0.30 * micro + 0.70 * wall_mid) / 2.
  - R4-V5200-F01: V_5200 vol-cluster MM widening - track 100-tick rolling
        realised vol per strike; widen edge_shells +2 when current vol
        exceeds the day-median rolling-vol over the trailing 1000 obs.

Persists vol_state_5200 in traderData (rolling mid history + rolling vol
window). Other state (smile, mark_lean, fs) preserved from Probe 1.
...
# ---------- voucher microstructure constants (NEW) -------------------------

# R4-V5000-C02: imb_k2 -> fair skew on V_5000 (capped at +/-1 tick)
V5000_IMB_BETA = 3.0
V5000_IMB_CAP = 1.0

# R4-V5100-C01: microprice/wall_mid blend at 50/50 with smile fair on V_5100
V5100_MICRO_W = 0.30
V5100_WALL_W = 0.70
V5100_BLEND_ALPHA = 0.5   # weight on the (micro+wall) blend vs smile-fair
WALL_VOL_THR_VOU = 5      # vouchers carry smaller volumes than HYD/VFE

# R4-V5200-F01: vol-cluster MM widening on V_5200
V5200_VOL_WIN = 100        # length of squared-return window
V5200_HIST_WIN = 1000      # length of rolling-vol history used for the median
V5200_EDGE_BONUS = 2       # extra edge_shells when current_vol > median
...
def _smile_voucher_orders(state, depths, positions, timestamp, ve_micro,
    ...
    for (k, m, _, v) in strike_data:
        ...
        # NEW: per-strike microstructure adjustments.
        if k == 5000:
            # R4-V5000-C02: imb_k2 -> fair skew, capped at +/-1 tick
            imb2 = _imb_k2(d_)
            v5000_skew = V5000_IMB_BETA * imb2
            if v5000_skew > V5000_IMB_CAP: v5000_skew = V5000_IMB_CAP
            if v5000_skew < -V5000_IMB_CAP: v5000_skew = -V5000_IMB_CAP
            fair = fair + v5000_skew
        elif k == 5100:
            # R4-V5100-C01: blend (micro/wall) at 50% with smile fair
            m_micro = _micro(d_)
            m_wall = _wall_mid(d_, WALL_VOL_THR_VOU)
            if m_micro is not None and m_wall is not None:
                blend_px = V5100_MICRO_W * m_micro + V5100_WALL_W * m_wall
                fair = (fair + blend_px) / 2.0
        ...
        # NEW: R4-V5200-F01 vol-cluster MM widening on V_5200.
        if k == 5200 and vol_state_5200 is not None and cur_mid is not None:
            # Update rolling-vol window (squared returns over the last 100 mids).
            mids_w = vol_state_5200.setdefault("mids", [])
            mids_w.append(cur_mid)
            if len(mids_w) > V5200_VOL_WIN + 1:
                # keep only last (V5200_VOL_WIN+1) mids -> V5200_VOL_WIN sq-returns
                del mids_w[0]
            cur_vol = None
            if len(mids_w) >= 2:
                ssq = 0.0
                for i in range(1, len(mids_w)):
                    r = mids_w[i] - mids_w[i - 1]
                    ssq += r * r
                cur_vol = math.sqrt(ssq / (len(mids_w) - 1))
            if cur_vol is not None:
                vol_hist = vol_state_5200.setdefault("vol_hist", [])
                vol_hist.append(cur_vol)
                if len(vol_hist) > V5200_HIST_WIN:
                    del vol_hist[0]
                if len(vol_hist) >= 50:
                    sorted_h = sorted(vol_hist)
                    median_v = sorted_h[len(sorted_h) // 2]
                    if cur_vol > median_v:
                        edge_shells = edge_shells + V5200_EDGE_BONUS
...
class Trader:
    def run(self, state: TradingState):
        ...
        # NEW: vol-cluster state for V_5200 (lists of mids and rolling vols)
        prior_vol_5200 = dict(prior.get("vol_state_5200", {}) or {})
        vol_state_5200: Dict[str, list] = {
            "mids": list(prior_vol_5200.get("mids") or []),
            "vol_hist": list(prior_vol_5200.get("vol_hist") or []),
        }
        ...
        # ---------- Phase 1 smile MM (with NEW microstructure tweaks) ----
        ...
        td_out = {
            ...
            "vol_state_5200": {
                "mids": [round(x, 4) for x in vol_state_5200.get("mids", [])],
                "vol_hist": [round(x, 4) for x in vol_state_5200.get("vol_hist", [])],
            },
        }
