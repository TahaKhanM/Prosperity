"""Phase 4.A — per-Mark inventory inference gate on top of composite v01.

Track running net inventory per (Mark, product) from the trade stream:
buys = +qty, sells = -qty. When applying the Mark-lean overlay, gate the
lean by inferred inventory: if `|inferred_inv[Mark, product]| > MARK_INV_THR`,
multiply that Mark's contribution by INV_GATE_REDUCTION (= 0.5).

Pre-registered (locked):
  MARK_INV_THR        = 50      # units per (Mark, product)
  INV_GATE_REDUCTION  = 0.5     # multiply lean by 0.5 when crossed
  INV_RESET_PER_DAY   = True    # reset Mark inventory at day boundary

Ship gate: 3-day Δ vs composite v01 ≥ +500.
...
# Phase 4.A — per-Mark inventory gate constants
MARK_INV_THR = 50
INV_GATE_REDUCTION = 0.5
INV_TRACKED_MARKS = ("Mark 14", "Mark 38", "Mark 22", "Mark 49")
...
def _update_mark_lean(mark_lean: Dict[str, float],
                      mark_inv: Dict[str, Dict[str, int]],
                      market_trades, current_ts: int,
                      last_ts: int) -> None:
    """Decay lean, track per-Mark inventory, accumulate gated lean."""
    ...
    for product, trades in (market_trades or {}).items():
        if product == HYD:
            ...
            # Still update inventory for tracked Marks even on non-rule products,
            # but no lean contribution.
            rules = {}
        for t in trades:
            ...
            # Update inferred inventory for tracked Marks.
            if buyer in INV_TRACKED_MARKS:
                bk = mark_inv.setdefault(buyer, {})
                bk[product] = bk.get(product, 0) + int(qty)
            if seller in INV_TRACKED_MARKS:
                sk = mark_inv.setdefault(seller, {})
                sk[product] = sk.get(product, 0) - int(qty)

            delta = 0.0
            if buyer in rules:
                w_buy, _ = rules[buyer]
                inv_b = mark_inv.get(buyer, {}).get(product, 0)
                if abs(inv_b) > MARK_INV_THR:
                    w_buy = w_buy * INV_GATE_REDUCTION
                delta += w_buy * qty * 0.05
            if seller in rules:
                _, w_sell = rules[seller]
                inv_s = mark_inv.get(seller, {}).get(product, 0)
                if abs(inv_s) > MARK_INV_THR:
                    w_sell = w_sell * INV_GATE_REDUCTION
...
class Trader:
    def run(self, state: TradingState):
        ...
        mark_inv: Dict[str, Dict[str, int]] = {
            k: dict(v) for k, v in (prior.get("mark_inv", {}) or {}).items()
        }
        last_ts = int(prior.get("last_ts", state.timestamp))
        prev_day = prior.get("inv_day")
        cur_day = int(state.timestamp) // 1_000_000
        if prev_day != cur_day:
            # INV_RESET_PER_DAY: clear inventory at day boundary.
            mark_inv = {}
        ...
        _update_mark_lean(mark_lean, mark_inv, market_trades,
