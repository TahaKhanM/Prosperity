"""Probe 4: r4_combined_v01 - probe 1 + probe 2 + probe 3 + 0.7 hedge ratio.

Combines:
  - Probe 1: per-product mark lean (HYD, VFE)
  - Probe 2: OTM AC1 K re-tune (K_5300=0.21, K_5400=0.25, K_5500=0.24)
  - Probe 3: cross-product VEV_5200 -> deep/ITM xprod lean
  - R4-CHAIN-01: portfolio-delta hedge with 0.70 sticky-strike adjustment.
    D = sum_K (Delta_BS * pos[K] * 0.70). If |D| > 40, post a take order on
    VFE to bring |D| down to 20. Never hedge with HYD.

Decay 0.985 per tick (HL ~46 ticks) for mark lean; 0.95 for xprod lean.
...
AC1_FADE_K_5300 = 0.21   # was 0.12 (R4-V5300-B07 retune)
AC1_FADE_K_5400 = 0.25   # was 0.15 (R4-V5400-B07)
AC1_FADE_K_5500 = 0.24   # was 0.15 (R4-V5500-B07)
AC1_FADE_CAP = 0.5
...
# ---------- mark lean constants (Probe 1) ----------------------------------

LEAN_DECAY = 0.985
LEAN_CLAMP_PROD = 3.0

# ---------- xprod lean constants (Probe 3) ---------------------------------

XPROD_LEAN_DECAY = 0.95
XPROD_LEAN_CLAMP = 5.0
XPROD_WEIGHTS = {
    4000: 1.0,
    4500: 0.7,
    5000: 0.3,
...
XPROD_TRIGGERS_BUYER = {"Mark 14"}
XPROD_TRIGGERS_SELLER = {"Mark 22"}
XPROD_PRODUCT = "VEV_5200"

# ---------- 0.7x Delta hedge constants (R4-CHAIN-01) -----------------------

HEDGE_DELTA_MULT = 0.70
HEDGE_THRESHOLD = 40.0   # |D| above this triggers hedge
HEDGE_TARGET = 20.0      # bring |D| down to this magnitude
...
MARK_RULES_VFE = {
    "Mark 14": (+0.10, -0.10),
    "Mark 38": (-0.10, +0.10),
    "Mark 67": (+0.10, 0.0),    # buy follow only
...
# ---------- xprod lean update (Probe 3) ------------------------------------


def _update_xprod_lean(xprod_lean: Dict[str, float], market_trades, current_ts: int,
                       last_ts: int) -> None:
    """Decay then accumulate per-strike xprod lean from VEV_5200 dyad prints."""
    elapsed_ticks = max(0, (current_ts - last_ts) // 100)
    if elapsed_ticks > 0:
        decay = XPROD_LEAN_DECAY ** elapsed_ticks
        for k in list(xprod_lean):
            xprod_lean[k] *= decay

    trades = (market_trades or {}).get(XPROD_PRODUCT, [])
    for t in trades:
        buyer = getattr(t, "buyer", None) or ""
        seller = getattr(t, "seller", None) or ""
        qty = float(getattr(t, "quantity", 0))
        if qty == 0:
            continue
        triggered = (buyer in XPROD_TRIGGERS_BUYER) or (seller in XPROD_TRIGGERS_SELLER)
        if not triggered:
            continue
        for k, w in XPROD_WEIGHTS.items():
            kstr = str(k)
            new_v = xprod_lean.get(kstr, 0.0) + w * qty * 0.10
            if new_v > XPROD_LEAN_CLAMP:
                new_v = XPROD_LEAN_CLAMP
            elif new_v < -XPROD_LEAN_CLAMP:
                new_v = -XPROD_LEAN_CLAMP
            xprod_lean[kstr] = new_v
...
def _smile_voucher_orders(state, depths, positions, timestamp, ve_micro, xprod_lean=None):
    out: Dict[str, List[Order]] = {}
    if xprod_lean is None:
        xprod_lean = {}
    ...
    for (k, m, _, v) in strike_data:
        ...
        # Probe 3 NEW: add xprod lean
        xpl = xprod_lean.get(str(k), 0.0)
        if xpl != 0.0:
            fair = fair + xpl
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in depths:
            ...
            pos = positions.get(sym, 0)
            intr = max(ref - k, 0.0)
            # Probe 3 NEW: bump intrinsic ref by xprod lean for deep-ITM strikes
            if k in ACC_STRIKES:
                xpl = xprod_lean.get(str(k), 0.0)
                if xpl != 0.0:
                    intr = intr + xpl
        ...
        # ---------- Phase 1 smile MM (unchanged + xprod) -----------------
        ...
        # ---------- 0.7x Delta portfolio hedge (R4-CHAIN-01) -------------
        # Compute portfolio delta from current voucher positions, using the
        # per-strike implied vol from the smile fit. Sticky-strike adjust 0.70.
        # If |D| > HEDGE_THRESHOLD, post a take order on VFE to bring |D| to
        # HEDGE_TARGET. Never hedge with HYD.
        if ve_micro is not None and ve_micro > 0 and vd is not None:
            t_yr = _tte_years(state.timestamp)
            if t_yr > 0:
                tte_days = t_yr * 365.0
                a0_prior, a1, a2 = _smile_prior(tte_days)
                ...
                if a0 is None:
                    a0 = a0_prior
                D = 0.0
                for k in ALL_VOUCHER_STRIKES:
                    sym = f"VEV_{k}"
                    pos_k = positions.get(sym, 0)
                    if pos_k == 0:
                        continue
                    bias_k = SMILE_PER_STRIKE_BIAS.get(k, 0.0)
                    m_log = math.log(ve_micro / float(k))
                    iv_k = a0 + a1 * m_log + a2 * m_log * m_log + bias_k
                    if iv_k <= 0.005:
                        iv_k = 0.05
                    delta_k = bs_call_delta(ve_micro, float(k), t_yr, iv_k)
                    D += delta_k * float(pos_k) * HEDGE_DELTA_MULT
                if abs(D) > HEDGE_THRESHOLD:
                    # We need to neutralise |D| - HEDGE_TARGET worth of delta.
                    # VFE proxy: 1 unit VFE delta=1 in same units as the
                    # underlying. Quantity to trade = sign(-D) * (|D| - target).
                    needed_qty = abs(D) - HEDGE_TARGET
                    bid = _bb(vd); ask = _ba(vd)
                    pos_vfe = positions.get(VFE, 0)
                    legs = list(out.get(VFE, []))
                    if D > 0 and bid is not None:
                        # long-delta: sell VFE
                        av = vd.buy_orders[bid]
                        sz = _cs(pos_vfe, CAP_V, 0, min(int(av), int(needed_qty)))
                        if sz > 0:
                            legs.append(Order(VFE, bid, -sz))
                    elif D < 0 and ask is not None:
                        # short-delta: buy VFE
                        av = -vd.sell_orders[ask]
                        sz = _cb(pos_vfe, CAP_V, 0, min(int(av), int(needed_qty)))
                        if sz > 0:
                            legs.append(Order(VFE, ask, sz))
                    if legs:
                        out[VFE] = legs
        ...
        td_out = {
            ...
            "xprod_lean": {k: round(v, 4) for k, v in xprod_lean.items()},
