"""Phase 2.B — V_5200 self-take on event spillover.

When a market_trade in VEV_5200 with qty ≥ EVENT_QTY_THR fires, infer the
aggressor side from the trade price relative to the prior tick's bid/ask,
and take VEV_5200 in the SAME direction (= follow the aggressor) for
EVENT_TAKE_QTY units. Hold passively; the existing voucher MM unwinds.

Different from the prior failed Phase 6 follow-up (`r4_xprod_v5200_event_take`):
that probe took VEV_4000 (21-tick spread devoured the +5-tick lift); this
takes V_5200 itself (~5-tick spread).

Pre-registered (locked):
  EVENT_QTY_THR        = 5     # 80th-percentile aggressive print qty
  EVENT_TAKE_QTY       = 10    # small relative to 300 cap
  EVENT_HORIZON_TICKS  = 50    # natural unwind horizon (informational; no
                                # explicit unwind code)
  EVENT_DAILY_CAP      = 30    # rate-limit per day

Equally-defensible: EVENT_QTY_THR ∈ [3,8]; EVENT_TAKE_QTY ∈ [5,15];
EVENT_HORIZON_TICKS ∈ [25,100].

Ship gate: 3-day Δ vs composite v01 ≥ +200 AND fires ≥ 10 times across 3
days AND day-3 Δ is the largest contributor.
...
# Phase 2.B — V_5200 self-take constants
EVENT_QTY_THR = 5
EVENT_TAKE_QTY = 10
EVENT_HORIZON_TICKS = 50
EVENT_DAILY_CAP = 30
V5200_SYM = "VEV_5200"
...
class Trader:
    def run(self, state: TradingState):
        ...
        # ---------- Phase 2.B — V_5200 self-take ------------------------
        prev_v5200_bid = prior.get("prev_v5200_bid")
        prev_v5200_ask = prior.get("prev_v5200_ask")
        prev_event_day = prior.get("event_day")
        event_take_count = int(prior.get("event_take_count", 0))
        cur_day = int(state.timestamp) // 1_000_000
        if prev_event_day != cur_day:
            event_take_count = 0
        v5200_d = depths.get(V5200_SYM)
        if v5200_d is not None:
            cur_v5200_bid = _bb(v5200_d)
            cur_v5200_ask = _ba(v5200_d)
            if (
                event_take_count < EVENT_DAILY_CAP
                and prev_v5200_bid is not None and prev_v5200_ask is not None
            ):
                trades = (market_trades or {}).get(V5200_SYM, []) or []
                # Sum buy/sell capacity already consumed by smile MM for V_5200.
                cur_legs = out.get(V5200_SYM, [])
                qb = sum(o.quantity for o in cur_legs if o.quantity > 0)
                qs = -sum(o.quantity for o in cur_legs if o.quantity < 0)
                pos_v = positions.get(V5200_SYM, 0)
                for tr in trades:
                    if event_take_count >= EVENT_DAILY_CAP:
                        break
                    qty = float(getattr(tr, "quantity", 0) or 0)
                    if qty < EVENT_QTY_THR:
                        continue
                    pr = float(getattr(tr, "price", 0) or 0)
                    if pr >= prev_v5200_ask:
                        # buyer-initiated; take ask side
                        if cur_v5200_ask is None:
                            continue
                        avail = -v5200_d.sell_orders.get(cur_v5200_ask, 0)
                        sz_max = min(EVENT_TAKE_QTY, avail)
                        sz = _cb(pos_v, CAP_VOU, qb, sz_max)
                        if sz > 0:
                            out.setdefault(V5200_SYM, []).append(
                                Order(V5200_SYM, cur_v5200_ask, sz))
                            qb += sz
                            event_take_count += 1
                    elif pr <= prev_v5200_bid:
                        # seller-initiated; take bid side (sell)
                        if cur_v5200_bid is None:
                            continue
                        avail = v5200_d.buy_orders.get(cur_v5200_bid, 0)
                        sz_max = min(EVENT_TAKE_QTY, avail)
                        sz = _cs(pos_v, CAP_VOU, qs, sz_max)
                        if sz > 0:
                            out.setdefault(V5200_SYM, []).append(
                                Order(V5200_SYM, cur_v5200_bid, -sz))
                            qs += sz
                            event_take_count += 1
                    # else: ambiguous — skip
            new_prev_v5200_bid = cur_v5200_bid if cur_v5200_bid is not None else prev_v5200_bid
            new_prev_v5200_ask = cur_v5200_ask if cur_v5200_ask is not None else prev_v5200_ask
        else:
            new_prev_v5200_bid = prev_v5200_bid
            new_prev_v5200_ask = prev_v5200_ask
        ...
        td_out = {
            ...
            "prev_v5200_bid": new_prev_v5200_bid,
            "prev_v5200_ask": new_prev_v5200_ask,
            "event_take_count": event_take_count,
            "event_day": cur_day,
            "smile": {
