"""H3 voucher-accumulate: long-bias deep-ITM vouchers. Buy aggressively below intrinsic+slack, hold to cap 300.
Hypothesis: 72k curve = sustained long inventory that benefits from compounding. Deep-ITM vouchers are bond-like."""
import json, math
...
# voucher long accumulation params
ACC_STRIKES = [4000, 4500]  # deep ITM = bond-like
ACC_SLACK = 4               # buy when ask <= intrinsic + 4
ACC_SIZE_PER_TICK = 50      # aggressive accumulation
EXIT_SLACK = 6              # sell only if bid >= intrinsic + 6
...
class Trader:
    def run(self, state):
        ...
        for sym in depths:
            ...
            legs = []
            if k in ACC_STRIKES:
                # LONG-BIAS deep ITM accumulation
                ...
                # exit when price spikes above intrinsic + EXIT_SLACK
