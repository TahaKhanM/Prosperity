"""Q2: INVERTED band - SELL when mid<9985 (trend continuation), BUY when mid>10015."""
import json, math
...
class Trader:
    def run(self, state):
        ...
        if hd is not None and HYD in fs:
            ...
            fair=fs[HYD]+HIMB*himb; cm=_mid(hd)
            legs=[]; b=s=0
            if cm is not None:
                dev=cm-ANCHOR
                # INVERTED: sell when below band (expect continuation down)
                if dev<=-BAND and bid is not None:
                    av=hd.buy_orders[bid]; sz=_cs(pos,CAPH,s,min(av,BSZ))
                    if sz>0: legs.append(Order(HYD,bid,-sz)); s+=sz
                elif dev>=BAND and ask is not None:
                    av=-hd.sell_orders[ask]; sz=_cb(pos,CAPH,b,min(av,BSZ))
