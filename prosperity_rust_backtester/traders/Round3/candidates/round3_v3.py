"""Round 3 v3 — v2 + voucher EMA own-mid MM + moderated anchor + regime guard.

Changes vs v2:
  - HYDROGEL anchor weight 0.85 -> 0.5.
    Empirical backtester sweep {0.3, 0.5, 0.7, 0.85} picks 0.5 at +132k/3d
    (vs +124k at 0.85) and is materially safer against a live HYDROGEL
    drift away from 10000 — the failure mode documented in the prior
    hosted submission 369524 (85% anchor lost 1.0k HYDROGEL PnL in a
    single hosted day because mid drifted).
  - HYDROGEL regime guard: if |EMA(micro, slow) - anchor| > 80 shells,
    the trader halts aggressive takes but keeps passive quotes. Drawdown
    brake that does not no-op.
  - Voucher near-ATM passive MM switches from v02-TV-proxy fair to
    EMA(own_mid, alpha=0.10) fair (sub-agent research, +10k / 3 days
    under a tight-fill sim vs +2k for the TV proxy). Strikes 5000-5300
    only.
  - Strike-monotonicity arb and identity take/upper-bound unchanged.
...
# ----- A1 HYDROGEL anchored MM --------------------------------------------
H_ANCHOR = 10000.0
H_ANCHOR_WEIGHT = 0.5           # v2: 0.85 -> v3: 0.5 (safer on hosted)
H_EWMA_ALPHA = 0.05
H_TAKE_EDGE = 1
H_QUOTE_EDGE = 4
H_SOFT_POS_LIMIT = 80
H_QUOTE_SIZE = 20
H_REGIME_GUARD_DEV = 80.0       # if |slow_micro - anchor| > this, halt takes
...
# ----- V2 voucher identity + strike-mono arb -------------------------------
VOUCHER_SOFT_CAP = 50
VOUCHER_TAKE_SLACK = 1
VOUCHER_UPPER_SLACK = 1
VOUCHER_MONO_SLACK = 1

# ----- V3 voucher EMA own-mid MM (sub-agent alpha #1) ----------------------
...
# ---------- A1 HYDROGEL (moderated anchor + regime guard) -----------------
...
def _run_hydrogel(state: Dict, depth: OrderDepth, position: int, imbalance: float, enable_skew: bool) -> List[Order]:
    ...
    if micro is None:
        return []
    # Slow EMA of micro for regime detection
    slow = state.get("h_slow")
    slow_alpha = 0.01
    if slow is None:
        slow = micro
    else:
        slow = slow * (1 - slow_alpha) + micro * slow_alpha
    state["h_slow"] = slow

    # Regime guard: if slow micro deviates from anchor, halt takes
    allow_take = abs(slow - H_ANCHOR) <= H_REGIME_GUARD_DEV
...
def _run_voucher_ema_mm(state: Dict, order_depths: Dict[str, OrderDepth],
    ...
    for k in VOUCHER_EMA_STRIKES:
        ...
        # take if ba+1 <= fair or bb-1 >= fair
        ...
        # passive quotes at fair +- edge, joined inside
