"""Round 3 v4 — post-mortem fix for hosted submission 378834 (−1,701 XY).

Root cause of the live loss: v3 hard-anchored HYDROGEL fair value toward
10 000 with weight 0.5. Live Round 3 HYDROGEL mid mean was **9 979.3**,
not 9 991 (historical) and not 10 000 (anchor). The 20-shell positive
bias pushed v3's bids above the live market, accumulated +200 HYDROGEL
at an average entry of 9 977, and marked-to-market at 9 960 at day-close
for a −3,341 HYDROGEL PnL (out of −1,701 total). Everything else in v3
was positive: VE +524, voucher EMA MM +1,085, identity arbs +50.

Fix in v4: HYDROGEL anchor is **learned from the market**, not hard-coded.
A slow EMA of microprice (alpha 0.005, half-life ~138 ticks, seeded at the
first observed microprice — NOT 10 000) tracks the session mean. Fair is
blended `0.3 * slow_anchor + 0.7 * microprice`, then EWMA-smoothed. This
keeps the variance-damping benefit of a session-mean anchor while letting
the anchor itself drift with the true market. Clamp kept at [9 940, 10 030]
as a defensive sanity gate.

Backtest comparison (both runs on the same Rust Legacy fill model):
  engine   | v3 anchor=10000,w=0.5 | v4 learned anchor,w=0.3
  replay   |  −1,657               |  +2,497   (+4,155 vs v3, −3,341 HYD → +813)
  local 3d | +142,254              | +40,714   (overfit evaporates; still > gate)

All non-HYDROGEL modules (B1 VE wall-mid MM, C1 imbalance skew, G1 parity
guard, V2 voucher identity + mono, V3 voucher EMA own-mid MM on strikes
5000–5300) are **unchanged** from v3 — they worked live and stay.

Single file, hosted-compatible.
"""
...
# ----- A1 HYDROGEL with LEARNED anchor -------------------------------------
# Anchor is no longer hard-coded to 10000. A slow EMA of microprice
# (alpha=0.005, half-life ~138 ticks) tracks the session mean. Seeded at the
# first observed microprice to avoid opening-day anchor bias. Clamp is
# defensive against a pathological mid excursion that would otherwise pull
# the anchor off-regime. Regime guard still halts aggressive takes when the
# slow EMA deviates from the clamped anchor (i.e., clamp is active).
H_SLOW_ALPHA = 0.005
H_ANCHOR_WEIGHT = 0.3            # share of slow-anchor vs raw micro
H_CLAMP_LO = 9940.0
H_CLAMP_HI = 10030.0
H_EWMA_ALPHA = 0.05
H_TAKE_EDGE = 1
...
def _run_hydrogel(state: Dict, depth: OrderDepth, position: int, imbalance: float, enable_skew: bool) -> List[Order]:
    ...
    if slow is None:
        slow = micro  # seed at observed microprice — no 10000 bias
    else:
    ...
    # Regime guard: if the clamp engaged (slow drifted outside sane range),
    # halt takes and keep only passive quotes. `|slow - anchor| > 0` means
    # the clamp is active; the 80-shell threshold tolerates minor clamping
    # noise but catches sustained drift.
    allow_take = abs(slow - anchor) <= H_REGIME_GUARD_DEV
    return _take_and_quote(HYDROGEL, depth, fair, position,
