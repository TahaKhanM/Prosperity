"""F6 — High-selectivity, high-size HYDROGEL strategy.

Only trade when imbalance >= 0.7 AND spread >= 10. When triggered, size 100.
Otherwise minimal passive quotes. Hypothesis: selectivity beats constant MM.
"""
import json, math
from typing import Dict, List, Optional
from datamodel import Order, OrderDepth, TradingState

HYD = "HYDROGEL_PACK"
VFE = "VELVETFRUIT_EXTRACT"
CAP_H = 200
CAP_V = 200
CAP_VOU = 300

ANCHOR = 10000.0
A_W = 0.40
A_HYD = 0.10
SELECT_IMB = 0.70
SELECT_SPREAD = 10
SELECT_SIZE = 100

SMALL_QUOTE_SIZE = 8
SMALL_QUOTE_EDGE = 4
HGUARD = 120.0

VSIZE = 30
A_VFE = 0.20
V_MKT_W = 0.7
V_MDL_W = 0.3
TV_A = 55.0
TV_W = 400.0
TV_POW = 1.8
TV_FLOOR = 0.5


def _bb(d): return max(d.buy_orders) if d.buy_orders else None
def _ba(d): return min(d.sell_orders) if d.sell_orders else None
def _mid(d):
    b, a = _bb(d), _ba(d)
    return None if b is None or a is None else (b + a) / 2.0
def _micro(d):
    b, a = _bb(d), _ba(d)
    if b is None or a is None: return None
    bv = d.buy_orders[b]; av = -d.sell_orders[a]
    t = bv + av
    return (b + a) / 2.0 if t <= 0 else (a * bv + b * av) / t
def _imb(d):
    b, a = _bb(d), _ba(d)
    if b is None or a is None: return 0.0
    bv = d.buy_orders[b]; av = -d.sell_orders[a]
    t = bv + av
    return 0.0 if t <= 0 else (bv - av) / t
def _strike(s):
    if s.startswith("VEV_"):
        try: return int(s[4:])
        except: return None
    return None
def _cb(p, cap, a, w): return max(0, min(w, cap - (p + a)))
def _cs(p, cap, a, w): return max(0, min(w, cap + (p - a)))
def _ew(p, s, a): return s if p is None else p * (1 - a) + s * a
def _tv(S, K):
    d = abs(S - K)
    if d >= TV_W: return TV_FLOOR
    x = 1.0 - d / TV_W
    return max(TV_FLOOR, TV_A * (x ** TV_POW)) if x > 0 else TV_FLOOR
def _vfair(S, K, vm):
    intr = max(S - K, 0.0)
    m = max(0.0, min(intr + _tv(S, K), S))
    if vm is None: return m
    return max(intr, min(V_MKT_W * vm + V_MDL_W * m, S))


class Trader:
    def run(self, state: TradingState):
        out = {}
        try: prior = json.loads(state.traderData) if state.traderData else {}
        except: prior = {}
        fs = dict(prior.get("fs", {}) or {})
        positions = state.position or {}
        depths = state.order_depths or {}

        hd = depths.get(HYD)
        if hd is not None:
            m = _micro(hd)
            if m is not None:
                blend = A_W * ANCHOR + (1 - A_W) * m
                fs[HYD] = _ew(fs.get(HYD), blend, A_HYD)
            imb = _imb(hd)
            bid = _bb(hd); ask = _ba(hd)
            spread = (ask - bid) if (bid is not None and ask is not None) else 0
            pos = positions.get(HYD, 0)
            legs = []
            # SELECTIVE HIGH-SIZE TAKE
            if abs(imb) >= SELECT_IMB and spread >= SELECT_SPREAD:
                if imb > 0 and ask is not None:
                    av = -hd.sell_orders[ask]
                    sz = _cb(pos, CAP_H, 0, min(av, SELECT_SIZE))
                    if sz > 0: legs.append(Order(HYD, ask, sz))
                elif imb < 0 and bid is not None:
                    av = hd.buy_orders[bid]
                    sz = _cs(pos, CAP_H, 0, min(av, SELECT_SIZE))
                    if sz > 0: legs.append(Order(HYD, bid, -sz))
            # baseline small passive (keeps from being pure-no-op)
            if HYD in fs:
                f = fs[HYD]
                cm = _mid(hd)
                if cm is None or abs(cm - f) <= HGUARD:
                    bp = int(math.floor(f - SMALL_QUOTE_EDGE))
                    sp = int(math.ceil(f + SMALL_QUOTE_EDGE))
                    if bid is not None: bp = min(bp, bid + 1)
                    if ask is not None: bp = min(bp, ask - 1); sp = max(sp, ask - 1)
                    if bid is not None: sp = max(sp, bid + 1)
                    inv = max(-1.0, min(1.0, pos / CAP_H))
                    bsz = _cb(pos, CAP_H, 0, max(0, int(round(SMALL_QUOTE_SIZE * max(0.0, 1 - 2.0 * inv)))))
                    ssz = _cs(pos, CAP_H, 0, max(0, int(round(SMALL_QUOTE_SIZE * max(0.0, 1 + 2.0 * inv)))))
                    if bsz > 0 and (ask is None or bp < ask): legs.append(Order(HYD, bp, bsz))
                    if ssz > 0 and (bid is None or sp > bid): legs.append(Order(HYD, sp, -ssz))
            if legs: out[HYD] = legs

        vd = depths.get(VFE)
        if vd is not None:
            m = _micro(vd)
            if m is not None:
                fs[VFE] = _ew(fs.get(VFE), m, A_VFE)
            pos = positions.get(VFE, 0)
            bid = _bb(vd); ask = _ba(vd)
            legs = []
            if VFE in fs:
                fair = fs[VFE]
                b = s = 0
                if ask is not None and ask + 1 <= fair:
                    av = -vd.sell_orders[ask]
                    sz = _cb(pos, CAP_V, b, min(av, CAP_V))
                    if sz > 0: legs.append(Order(VFE, ask, sz)); b += sz
                if bid is not None and bid - 1 >= fair:
                    av = vd.buy_orders[bid]
                    sz = _cs(pos, CAP_V, s, min(av, CAP_V))
                    if sz > 0: legs.append(Order(VFE, bid, -sz)); s += sz
                bp = int(math.floor(fair - 1))
                sp = int(math.ceil(fair + 1))
                if bid is not None: bp = min(bp, bid + 1)
                if ask is not None: bp = min(bp, ask - 1); sp = max(sp, ask - 1)
                if bid is not None: sp = max(sp, bid + 1)
                inv = max(-1.0, min(1.0, (pos + b - s) / CAP_V))
                bsz = _cb(pos, CAP_V, b, max(0, int(round(VSIZE * max(0.0, 1 - 1.5 * inv)))))
                ssz = _cs(pos, CAP_V, s, max(0, int(round(VSIZE * max(0.0, 1 + 1.5 * inv)))))
                if bsz > 0 and (ask is None or bp < ask): legs.append(Order(VFE, bp, bsz))
                if ssz > 0 and (bid is None or sp > bid): legs.append(Order(VFE, sp, -ssz))
            if legs: out[VFE] = legs

        ref = fs.get(VFE)
        if ref is None and vd is not None: ref = _mid(vd)
        for sym in depths:
            if sym in (HYD, VFE): continue
            k = _strike(sym)
            if k is None or not (4000 <= k <= 6500): continue
            d_ = depths.get(sym)
            if d_ is None or ref is None: continue
            bid = _bb(d_); ask = _ba(d_)
            pos = positions.get(sym, 0)
            intr = max(ref - k, 0.0)
            legs = []
            # identity only
            if ask is not None and ask + 1 < intr:
                av = -d_.sell_orders[ask]
                sz = _cb(pos, CAP_VOU, 0, min(av, CAP_VOU))
                if sz > 0: legs.append(Order(sym, ask, sz))
            if bid is not None and bid - 1 > ref:
                av = d_.buy_orders[bid]
                sz = _cs(pos, CAP_VOU, 0, min(av, CAP_VOU))
                if sz > 0: legs.append(Order(sym, bid, -sz))
            if legs: out[sym] = legs

        blob = {"fs": {k: round(v, 4) for k, v in fs.items()}}
        return out, 0, json.dumps(blob, separators=(",", ":"))

    def bid(self): return 20
