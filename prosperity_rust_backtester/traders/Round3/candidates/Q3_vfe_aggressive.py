"""Q3: baseline HYDROGEL MM + aggressive VFE at size 100 edge 0 (step inside)."""
import json, math
    ...
    def run(self, state):
        ...
        if hd is not None and HYD in fs:
            ...
            if ask is not None and ask+HTAKE<=fair:
                av=-hd.sell_orders[ask]; sz=_cb(pos,CAPH,b,min(av,CAPH))
        ...
        # VFE AGGRESSIVE: size 100, step inside
        if vd is not None and VFE in fs:
            ...
            # inside-book quotes
            bp = bid+1 if bid is not None else int(math.floor(fair-1))
            sp = ask-1 if ask is not None else int(math.ceil(fair+1))
            if ask is not None: bp=min(bp,ask-1)
            if bid is not None: sp=max(sp,bid+1)
            bsz=_cb(pos,CAPV,b,max(0,int(round(100*max(0.0,1-1.5*inv)))))
            ssz=_cs(pos,CAPV,s,max(0,int(round(100*max(0.0,1+1.5*inv)))))
