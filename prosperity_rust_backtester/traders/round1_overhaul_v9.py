...
def ash_fair_and_state(
    ...
    current_wall = wall_mid(depth) or current_mid or ASH_ANCHOR
    book_mid = mm10_mid(depth) or current_wall or current_mid or ASH_ANCHOR
    ...
    fair = (
        10000.10
        + 0.59 * (book_mid - ASH_ANCHOR)
        + 0.10 * (current_wall - ASH_ANCHOR)
        + 0.29 * (current_micro - ASH_ANCHOR)
        + 0.07 * imbalance_now
        - 0.11 * ret1
    )
    fair = clamp(fair, ASH_ANCHOR - 8.0, ASH_ANCHOR + 8.0)
    signal = fair - current_mid
    shock = abs(signal) >= 2.2 or abs(ret1) >= 3.5
...
def trade_ash(
    ...
    target_position = int(round(clamp(signal * 8.0, -24.0, 24.0)))
    long_cap = 74
    short_cap = 74
    soft_cap = 58

    take_edge = max(2.7, 3.6 - min(1.1, abs(signal) * 0.45))
    clear_edge = 0.35
    skew = clamp((position - target_position) * 0.06, -2.4, 2.4)
    ...
    upper_soft = max(soft_cap, target_position + 24)
    lower_soft = min(-soft_cap, target_position - 24)
    if temp_position > upper_soft:
        for bid in sorted(depth.buy_orders, reverse=True):
    ...
    buy_prices = [
        math.floor(quote_fair - (4.2 if signal >= 0.0 else 4.8)),
        math.floor(quote_fair - 6.4),
    ]
    sell_prices = [
        math.ceil(quote_fair + (4.2 if signal <= 0.0 else 4.8)),
        math.ceil(quote_fair + 6.4),
    ]
    buy_sizes = [8, 10] if not shock else [6, 8]
    sell_sizes = [8, 10] if not shock else [6, 8]
    if signal > 1.0:
        buy_sizes = [size + 2 for size in buy_sizes]
        sell_sizes = [max(5, size - 2) for size in sell_sizes]
    elif signal < -1.0:
        sell_sizes = [size + 2 for size in sell_sizes]
        buy_sizes = [max(5, size - 2) for size in buy_sizes]
