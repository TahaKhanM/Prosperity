...
ASH_FAIR = 10000.0
PEPPER_LOOKAHEAD = 900.0
PEPPER_MAX_TREND_BIAS = 4.0
PEPPER_IMBALANCE_WEIGHT = 1.6
PEPPER_WALL_WEIGHT = 0.9
PEPPER_MAX_PRESSURE_BIAS = 2.5
...
def pepper_fair_with_session_bias(
    depth: OrderDepth,
    ...
    session_slope = (current_wall - float(session_state["open"])) / float(elapsed)
    trend_bias = clamp(
        session_slope * PEPPER_LOOKAHEAD,
        -PEPPER_MAX_TREND_BIAS,
        PEPPER_MAX_TREND_BIAS,
    )

    current_mid = current_mid or current_wall
    pressure_bias = clamp(
        (top_imbalance(depth) * PEPPER_IMBALANCE_WEIGHT)
        + ((current_wall - current_mid) * PEPPER_WALL_WEIGHT),
        -PEPPER_MAX_PRESSURE_BIAS,
        PEPPER_MAX_PRESSURE_BIAS,
    )

    session_state["last_ts"] = timestamp
    fair = current_wall + trend_bias + pressure_bias
...
class Trader:
    def run(self, state: TradingState):
        ...
        if PEPPER in state.order_depths:
            pepper_fair, session_state = pepper_fair_with_session_bias(
