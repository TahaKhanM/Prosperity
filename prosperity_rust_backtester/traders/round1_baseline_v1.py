    ...
    def run(self, state: TradingState):
        ...
        if ASH in state.order_depths:
            result[ASH] = trade_product(
                ...
                BASE_CONFIG[ASH]["fair"],
                BASE_CONFIG[ASH],
            )

        if PEPPER in state.order_depths:
            pepper_fair = wall_mid(state.order_depths[PEPPER])
            if pepper_fair is None:
                pepper_fair = simple_mid(state.order_depths[PEPPER])
            if pepper_fair is None:
                pepper_fair = float(pepper_state.get("last_fair", 11000.0))
            pepper_state["last_fair"] = pepper_fair
        ...
        trader_data = json.dumps({"pepper": pepper_state}) if pepper_state else ""
