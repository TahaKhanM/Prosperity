"""Temp Round 2 sweep variant 10: Pepper quote-side/noquote gate."""
...
def trade_pepper(
    ...
    passive_buy_gate = (
        temp_position >= 76
        and regime_progress >= 0.33
        and residual >= 1.5
        and not buyflow
    )
    if passive_buy_gate:
        bid_edge += 1.0
        ask_edge = max(3.6, ask_edge - 0.6)
    ...
    if passive_buy_gate:
        buy_sizes[0] = max(0, int(round(buy_sizes[0] * 0.25)))
