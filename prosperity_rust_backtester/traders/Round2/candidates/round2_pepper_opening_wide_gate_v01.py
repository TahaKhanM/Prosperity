"""Round 2 candidate: baseline Pepper shell with an early opening wide-spread take gate."""
...
def trade_pepper(
    ...
    opening_wide_buy_gate = (
        mode == "opening"
        and live_spread is not None
        and live_spread >= 12
        and regime_progress <= 0.03
        and position < max(60, target_long + 10)
        and not buyflow
    )

    if opening_wide_buy_gate:
        sweep_pad = min(sweep_pad, 0.5)
        buy_edge += 0.55
        bid_edge += 0.25
    ...
    pepper_state["diag_gate_mask"] = (
        ...
        | (32 if opening_wide_buy_gate else 0)
