"""Round 2 candidate v02: higher-carry dynamic-fair Pepper + baseline Ash."""
...
def trade_pepper(
    ...
    base_target = 80 if progress < 0.84 else 76 if progress < 0.95 else 62
    ...
    if buyflow:
        ...
        target_long -= 2
    if carry_left < 120.0:
        target_long = min(target_long, 70)
    target_long = int(clamp(target_long, 48, 80))
    ...
    for ask in sorted(depth.sell_orders):
        ...
        if temp_position < target_long - 8:
            buy_ceiling = max(buy_ceiling, fair + refill_edge + 0.6)
        if temp_position >= 78 and residual > -0.5:
    ...
    for bid in sorted(depth.buy_orders, reverse=True):
        ...
        if temp_position > target_long:
            sell_threshold = min(sell_threshold, fair + 2.0)
        if carry_left < 120.0:
            sell_threshold = min(sell_threshold, fair + 1.2)
    ...
    bid = top_bid
    ask = top_ask
    skew = clamp((temp_position - target_long) * 0.028, -1.5, 1.5)
    ...
    buy_sizes = [bid_size, max(8, bid_size - 4)]
    sell_sizes = [ask_size, max(3, ask_size - 2)]
    ...
    if temp_position > target_long + 2 or mode == "sell_rip" or sellflow or carry_left < 90.0:
        seen.clear()
