...
def pepper_state_and_fair(
    ...
    drift_confidence = 0.54 * gain_score + 0.28 * flow_score + 0.18 * slope_score
    ...
    fair = (
        0.35 * template_fair
        + 0.65 * book_fair
        + 0.06 * max(flow_bias, 0.0)
    )
    fair = max(fair, fallback - 0.15)

    pepper_state["last_mid"] = current_mid
    pepper_state["trade_ema"] = trade_ema
    pepper_state["generator_terminal"] = generator_terminal

    if regime_progress < 1.01 and drift_confidence >= 0.35 and residual < 1000.0:
        mode = "opening"
    elif residual < -2.0:
        mode = "discount"
    elif residual > 5.5:
...
def trade_pepper(
    ...
    if mode == "opening":
        target_long = min(target_long, 36 + int(round(12.0 * drift_confidence)))
    elif mode == "carry":
        target_long = min(target_long, 68 + int(round(10.0 * drift_confidence)))
    elif mode == "discount":
        target_long = min(target_long, 74 + int(round(6.0 * drift_confidence)))

    if drift_confidence < 0.45:
        buy_edge += 0.12 + 0.18 * (0.45 - drift_confidence)
        bid_edge += 0.10 + 0.14 * (0.45 - drift_confidence)
        if position > target_long:
            sell_edge = max(3.8, sell_edge - 0.18)
            ask_edge = max(4.8, ask_edge - 0.22)
