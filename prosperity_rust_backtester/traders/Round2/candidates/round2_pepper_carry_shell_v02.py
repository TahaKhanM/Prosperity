"""Round 2 candidate: baseline Pepper carry shell with explicit post-harvest target clamp."""
...
PEPPER_LATE_RECYCLE_STREAK = 14
PEPPER_DEEP_RECYCLE_STREAK = 22
PEPPER_RICH_HARVEST_RESIDUAL = 4.6
...
def trade_pepper(
    ...
    last_harvest_residual = float(pepper_state.get("last_harvest_residual", 0.0))
    last_harvest_cap_streak = int(pepper_state.get("last_harvest_cap_streak", 0))
    last_harvest_target_long = int(pepper_state.get("last_harvest_target_long", 80))
    ...
    late_cap_recycle = (
        position >= 78
        and residual >= 4.0
        and regime_progress >= 0.38
        and trade_ema <= 1.5
        and cap_streak >= PEPPER_LATE_RECYCLE_STREAK
    )
    deep_cap_recycle = (
        late_cap_recycle
        and (cap_streak >= PEPPER_DEEP_RECYCLE_STREAK or carry_left < 190.0)
    )
    if recycle_mode:
    ...
    if residual >= 4.0 and sellflow:
        ...
        ask_size += 1
    if late_cap_recycle:
        target_long = min(target_long, 74 if regime_progress < 0.66 else 73)
        sell_edge = max(3.35, sell_edge - 0.35)
        ask_edge = max(4.1, ask_edge - 0.4)
        ask_size = max(ask_size, 4)
    if deep_cap_recycle:
        target_long = min(target_long, 72 if regime_progress < 0.66 else 70)
        sell_edge = max(3.1, sell_edge - 0.55)
        ask_edge = max(3.8, ask_edge - 0.65)
        ask_size = max(ask_size, 5)

    refill_guard_active = (
        timestamp <= refill_guard_until
        and last_harvest_qty > 0
        and position >= max(70, target_long - 5)
    )
    strict_refill_guard = (
        refill_guard_active
        and (
            last_harvest_cap_streak >= PEPPER_LATE_RECYCLE_STREAK
            or last_harvest_residual >= PEPPER_RICH_HARVEST_RESIDUAL
            or position >= 78
        )
    )
    guard_reentry_target = max(68, min(80, last_harvest_target_long))
    guard_target_clamp_active = strict_refill_guard and position >= guard_reentry_target
    if guard_target_clamp_active:
        pullback_finish_buy = False
        target_long = min(target_long, guard_reentry_target)
        buy_edge += 0.55
        bid_edge += 0.35
        bid_size = max(4, bid_size - 4)
    top_bid = best_bid(depth)
    top_ask = best_ask(depth)
    ...
    pepper_state["diag_gate_mask"] = (
        ...
        | (8 if late_cap_recycle else 0)
        | (16 if refill_guard_active else 0)
        | (32 if strict_refill_guard else 0)
        | (64 if wide_high_inventory_guard else 0)
        | (128 if deep_cap_recycle else 0)
        | (256 if guard_target_clamp_active else 0)
    )
    ...
    for ask in sorted(depth.sell_orders):
        ...
        if wide_high_inventory_guard and temp_position >= 74:
            ...
            if refill_guard_active:
                extra_discount += 0.45
            if strict_refill_guard:
                extra_discount += 0.30
            if temp_position >= 78:
                extra_discount += 0.25
            buy_ceiling = min(buy_ceiling, fair - (buy_edge + extra_discount))
        if refill_guard_active:
            required_rebuy_gap = 3.0 if strict_refill_guard else 2.0
            stronger_discount_gap = 1.35 if strict_refill_guard else 1.0
            continuation_floor = 0.85 if strict_refill_guard else 0.55
            materially_cheaper = ask <= last_harvest_px - required_rebuy_gap
            stronger_discount = residual <= (-2.9 if strict_refill_guard else -2.5) or ask <= fair - (
                buy_edge + stronger_discount_gap
            )
            continuation_override = (
                buyflow
                and diagnostics["recent_slope"] >= (1.0 if strict_refill_guard else 0.8)
                and ask <= fair - max(continuation_floor, buy_edge - 0.2)
            )
            inventory_reset = temp_position <= max(
                60 if strict_refill_guard else 64,
                min(target_long, guard_reentry_target) - 6 if strict_refill_guard else target_long - 8,
            )
    ...
    if carry_left < 220.0:
        recycle_trigger -= 0.3
    if late_cap_recycle:
        recycle_trigger -= 0.25
    if deep_cap_recycle:
        recycle_trigger -= 0.30
    ...
    if discretionary_sell_qty > 0 and position >= 76 and (
        recycle_mode or recycle_exit_push or late_cap_recycle or mode == "overheated"
    ):
        guard_ticks = PEPPER_REFILL_GUARD_TICKS
        if late_cap_recycle:
            guard_ticks += 250
        if deep_cap_recycle:
            guard_ticks += 250
        ...
        pepper_state["last_harvest_residual"] = residual
        pepper_state["last_harvest_cap_streak"] = cap_streak
        pepper_state["last_harvest_target_long"] = max(
            68,
            min(80, target_long - (2 if deep_cap_recycle else 0)),
        )
        pepper_state["refill_guard_until"] = timestamp + guard_ticks
    elif timestamp > refill_guard_until or position <= max(64, target_long - 8):
        pepper_state["last_harvest_qty"] = 0
        pepper_state["last_harvest_residual"] = 0.0
        pepper_state["last_harvest_cap_streak"] = 0
        pepper_state["last_harvest_target_long"] = 80
    ...
    if late_cap_recycle:
        quote_fair = min(quote_fair, template_fair + 0.8)
    if deep_cap_recycle:
        quote_fair = min(quote_fair, template_fair + 0.35)
    buy_prices = [
