"""Round 2 candidate: baseline Pepper shell with a regularized generator fallback."""
...
PEPPER_GENERATOR_POWER = 0.62
PEPPER_GENERATOR_ALPHA = 0.08
...
def pepper_generator_curve(progress: float) -> float:
    return clamp(progress, 0.0, 1.0) ** PEPPER_GENERATOR_POWER
...
def pepper_state_and_fair(
    ...
    drift_curve = max(0.05, pepper_generator_curve(template_progress))
    close_curve = max(0.20, pepper_generator_curve(PEPPER_TEMPLATE_END_PROGRESS))
    observed_gain = clamp(current_mm - open_mid, -60.0, 1200.0)
    implied_terminal = observed_gain / drift_curve
    prev_terminal = float(pepper_state.get("generator_terminal", max(0.0, observed_gain)))
    generator_terminal = (
        (1.0 - PEPPER_GENERATOR_ALPHA) * prev_terminal
        + PEPPER_GENERATOR_ALPHA * max(0.0, implied_terminal)
    )
    generator_terminal = clamp(generator_terminal, 0.0, 1200.0)
    generator_fair = open_mid + generator_terminal * drift_curve
    stationary_fair = current_mm + 0.22 * micro_bias

    gain_score = clamp((observed_gain - 25.0) / 160.0, 0.0, 1.0)
    flow_score = clamp((trade_ema + 5.0) / 14.0, 0.0, 1.0)
    slope_score = clamp((recent_slope + 0.8) / 2.4, 0.0, 1.0)
    drift_confidence = 0.58 * gain_score + 0.24 * flow_score + 0.18 * slope_score
    if template_progress < 0.05 and observed_gain < 18.0:
        drift_confidence *= 0.45
    if trade_ema <= -6.0 or recent_slope <= -1.6:
        drift_confidence *= 0.45
    drift_confidence = clamp(drift_confidence, 0.0, 1.0)

    template_fair = (
        drift_confidence * generator_fair
        + (1.0 - drift_confidence) * stationary_fair
    )
    close_estimate = open_mid + generator_terminal * close_curve
    ...
    fair = (
        0.70 * template_fair
        + 0.30 * book_fair
        + 0.10 * max(flow_bias, 0.0)
    )
    fair = max(fair, fallback - 0.25)

    pepper_state["last_mid"] = current_mid
    pepper_state["trade_ema"] = trade_ema
    pepper_state["generator_terminal"] = generator_terminal

    if regime_progress < 1.01 and drift_confidence >= 0.48 and residual < 1000.0:
        mode = "opening"
    elif residual < -2.0:
        mode = "discount"
    elif residual > 5.5:
...
def trade_pepper(
    ...
    drift_confidence = float(diagnostics.get("drift_confidence", 1.0))

    if mode == "opening":
        target_long = min(target_long, 24 + int(round(28.0 * drift_confidence)))
    elif mode == "carry":
        target_long = min(target_long, 44 + int(round(34.0 * drift_confidence)))
    elif mode == "discount":
        target_long = min(target_long, 54 + int(round(26.0 * drift_confidence)))

    if drift_confidence < 0.55:
        buy_edge += 0.20 + 0.40 * (0.55 - drift_confidence)
        bid_edge += 0.15 + 0.25 * (0.55 - drift_confidence)
        if position > target_long:
            sell_edge = max(3.8, sell_edge - 0.30)
            ask_edge = max(4.6, ask_edge - 0.35)
            ask_size = max(ask_size, 2)
    ...
    pepper_state["diag_drift_confidence"] = round(drift_confidence, 3)
