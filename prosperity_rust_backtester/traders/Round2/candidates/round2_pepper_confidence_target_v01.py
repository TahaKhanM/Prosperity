"""Round 2 candidate: baseline Pepper shell with confidence-scaled carry concentration."""
...
def pepper_confidence_target(
    *,
    ...
    recent_slope: float,
    trade_ema: float,
    live_spread: Optional[int],
    regime_progress: float,
) -> int:
    if mode not in {"opening", "carry", "discount"}:
        return target_long
    ...
    if trade_ema >= 4.0:
        confidence += 0.20
    if live_spread is not None and live_spread <= 7:
        confidence += 0.10
    if regime_progress >= 0.20 and mode == "opening":
        confidence += 0.10
    if trade_ema <= -4.0:
        confidence -= 0.25
    if recent_slope <= -1.0:
    ...
    if mode == "opening":
        core, satellite = 28, 24
    elif mode == "discount":
        core, satellite = 62, 18
    else:
        core, satellite = 60, 18

    scaled = int(round(core + satellite * confidence))
    return max(0, min(80, min(target_long, scaled)))
...
def trade_pepper(
    ...
    confidence_target = pepper_confidence_target(
        mode=mode,
        target_long=target_long,
        residual=residual,
        recent_slope=diagnostics["recent_slope"],
        trade_ema=trade_ema,
        live_spread=live_spread,
        regime_progress=regime_progress,
    )
    if confidence_target < target_long:
        target_long = confidence_target
        buy_edge += 0.25
        bid_edge += 0.20
        if position > target_long:
            sell_edge = max(3.8, sell_edge - 0.35)
            ask_edge = max(5.0, ask_edge - 0.45)
    ...
    pepper_state["diag_confidence_target"] = confidence_target
