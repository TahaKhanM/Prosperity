...
def trade_ash(
    ...
    if temp_position >= ASH_EMERGENCY_POS and conviction == "weak":
        emergency_qty = min(int(depth.buy_orders[bid]), max(0, temp_position - 55), sell_budget, 12)
        if emergency_qty > 0:
            sell_budget = add_sell(orders, ASH, bid, emergency_qty, sell_budget)
            temp_position -= emergency_qty
            used_taker = True
    elif temp_position <= -ASH_EMERGENCY_POS and conviction == "weak":
        emergency_qty = min(int(-depth.sell_orders[ask]), max(0, -55 - temp_position), buy_budget, 12)
        if emergency_qty > 0:
    ...
    if progress >= 0.992 and temp_position > flatten_target:
        recycle_qty = min(int(depth.buy_orders[bid]), temp_position - flatten_target, sell_budget, 8)
        if recycle_qty > 0:
            sell_budget = add_sell(orders, ASH, bid, recycle_qty, sell_budget)
            temp_position -= recycle_qty
            used_taker = True
    elif progress >= 0.992 and temp_position < -flatten_target:
        recycle_qty = min(int(-depth.sell_orders[ask]), -temp_position - flatten_target, buy_budget, 8)
        if recycle_qty > 0:
            buy_budget = add_buy(orders, ASH, ask, recycle_qty, buy_budget)
            temp_position += recycle_qty
            used_taker = True
    ...
    # Late-day inventory control: stop rebuilding the book and lean passively toward flat.
    if progress >= 0.90 and temp_position >= flatten_target:
        ask_size = max(ask_size, 22)
        bid_size = min(bid_size, 6)
        my_ask -= 1
    if progress >= 0.90 and temp_position <= -flatten_target:
        bid_size = max(bid_size, 22)
        ask_size = min(ask_size, 6)
        my_bid += 1
    if progress >= 0.94 and temp_position >= 24:
        ask_size = max(ask_size, 26)
        bid_size = 0
        my_ask -= 1
    if progress >= 0.94 and temp_position <= -24:
        bid_size = max(bid_size, 26)
        ask_size = 0
        my_bid += 1
    if progress >= 0.975 and temp_position >= 12:
