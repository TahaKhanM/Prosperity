...
def trade_ash(
    ...
    buy_budget = max(0, min(LIMITS[ASH], ASH_ACTIVE_POS_LIMIT) - position)
    sell_budget = max(0, position + min(LIMITS[ASH], ASH_ACTIVE_POS_LIMIT))
    ...
    if temp_position >= ASH_ACTIVE_POS_LIMIT:
        bid_size = 0
        deep_bid_size = 0
    if temp_position <= -ASH_ACTIVE_POS_LIMIT:
        ask_size = 0
