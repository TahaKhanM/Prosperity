...
def trade_ash(
    ...
    shock_flat_sell_partial_take = (
        sell_follow_signal
        and spread == 10
        and shock
        and top_imbalance <= -0.44
        and micro_gap <= -2.0
        and diag["ret1"] >= 3.0
        and -19 <= temp_position <= 19
        and not (progress >= 0.88 and temp_position <= -12)
    )
    ...
    did_shock_sell_partial_take = False
    if best_bid_level is not None and shock_flat_sell_partial_take:
        before = sell_budget
        sell_budget = add_sell(
            orders,
            ASH,
            best_bid_level,
            min(6, depth.buy_orders[best_bid_level]),
            sell_budget,
        )
        temp_position -= before - sell_budget
        did_shock_sell_partial_take = before != sell_budget

    if (
        best_bid_level is not None
        and not did_shock_sell_partial_take
        and allow_sell_take_at(best_bid_level)
    ):
        before = sell_budget
        sell_budget = add_sell(
            orders,
