...
def trade_ash(
    ...
    wide_buy_size_signal = (
        spread >= 18
        and top_imbalance >= 0.30
        and micro_gap >= 0.45
        and (current_mid <= ASH_ANCHOR - 1.0 or fair <= ASH_ANCHOR - 1.0)
    )
    wide_sell_size_signal = (
        spread >= 18
        and top_imbalance <= -0.30
        and micro_gap <= -0.45
        and (current_mid >= ASH_ANCHOR + 1.0 or fair >= ASH_ANCHOR + 1.0)
    ...
    if (
        ...
        buy_sizes[0] += 4
        sell_sizes[0] = max(7, sell_sizes[0] - 1)
    if (
        wide_sell_size_signal
        and temp_position > -max(36, soft_cap + 16)
        and not one_sided_buy_follow
        and not (progress >= 0.88 and temp_position <= -18)
    ):
        sell_sizes[0] += 4
        buy_sizes[0] = max(7, buy_sizes[0] - 1)
