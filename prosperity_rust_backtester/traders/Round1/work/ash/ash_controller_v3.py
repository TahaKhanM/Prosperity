...
def post_passive_buy(
    ...
    *,
    ...
    if price is None:
        return budget
    return add_buy(orders, ASH, price, size, budget)
...
def post_passive_sell(
    ...
    *,
    ...
    if price is None:
        return budget
    return add_sell(orders, ASH, price, size, budget)
...
def post_cross_buy(
    ...
    *,
    ...
    before = budget
    budget = add_buy(orders, ASH, ask, size, budget)
...
def post_cross_sell(
    ...
    *,
    ...
    before = budget
    budget = add_sell(orders, ASH, bid, size, budget)
    return budget, before - budget
...
def choose_bucket(ctx: Dict[str, float], position: int) -> str:
    if ctx["ask_only"] and position > -72:
        return "ask_only"
    if ctx["bid_only"] and position < 72:
        return "bid_only"
    if ctx["anchor_dev"] >= 4.0 and ctx["pressure"] <= -0.18:
        return "rich_sell_extreme"
    if ctx["anchor_dev"] <= -4.0 and ctx["pressure"] >= 0.18:
        return "cheap_buy_extreme"
    if position >= 60 and ctx["pressure"] > -0.15:
        return "recycle_long"
    if position <= -60 and ctx["pressure"] < 0.15:
        return "recycle_short"
    if ctx["anchor_dev"] >= 2.0 and ctx["pressure"] <= -0.12:
        return "rich_sell"
    if ctx["anchor_dev"] <= -2.0 and ctx["pressure"] >= 0.12:
        return "cheap_buy"
    return "neutral"
...
def choose_action(bucket: str, ctx: Dict[str, float], position: int) -> str:
    buy_edge = max(0.0, -ctx["anchor_dev"] - 2.0) / 3.0 + max(0.0, ctx["pressure"]) + 0.22 * ctx["buy_run"]
    sell_edge = max(0.0, ctx["anchor_dev"] - 2.0) / 3.0 + max(0.0, -ctx["pressure"]) + 0.22 * ctx["sell_run"]

    scores = {
        "passive_two_sided": 1.15 - 0.55 * abs(ctx["anchor_dev"]) - 0.90 * abs(ctx["pressure"]),
        "passive_buy": buy_edge - 0.015 * max(position, 0),
        "passive_sell": sell_edge - 0.015 * max(-position, 0),
        "cross_buy": buy_edge - 0.95 - 0.020 * max(position, 0),
        "cross_sell": sell_edge - 0.95 - 0.020 * max(-position, 0),
        "hold": 0.40,
    }

    if bucket == "ask_only":
        scores["passive_two_sided"] -= 2.0
        scores["passive_sell"] += 0.95
        scores["cross_sell"] += 0.30
    elif bucket == "bid_only":
        scores["passive_two_sided"] -= 2.0
        scores["passive_buy"] += 0.95
        scores["cross_buy"] += 0.30
    elif bucket == "rich_sell_extreme":
        scores["passive_sell"] += 0.45
        scores["cross_sell"] += 0.65
    elif bucket == "cheap_buy_extreme":
        scores["passive_buy"] += 0.45
        scores["cross_buy"] += 0.65
    elif bucket == "rich_sell":
        scores["passive_sell"] += 0.25
        scores["cross_sell"] += 0.20
    elif bucket == "cheap_buy":
        scores["passive_buy"] += 0.25
        scores["cross_buy"] += 0.20
    elif bucket == "recycle_long":
        scores["passive_two_sided"] -= 0.9
        scores["passive_sell"] += 0.40
        scores["cross_sell"] += 0.35
        scores["hold"] += 0.15
    elif bucket == "recycle_short":
        scores["passive_two_sided"] -= 0.9
        scores["passive_buy"] += 0.40
        scores["cross_buy"] += 0.35
        scores["hold"] += 0.15

    if bucket == "neutral" and abs(position) < 20:
        scores["passive_two_sided"] += 0.40
    if ctx["spread"] < 2:
        scores["passive_two_sided"] -= 0.35
        scores["cross_buy"] -= 0.20
        scores["cross_sell"] -= 0.20
    if position >= 55:
        scores["cross_buy"] -= 1.2
    if position <= -55:
        scores["cross_sell"] -= 1.2
...
def trade_ash(
    ...
    fair, ash_state, ctx = ash_fair_context(depth, ash_state)
    buy_budget = max(0, LIMITS[ASH] - position)
    sell_budget = max(0, LIMITS[ASH] + position)
    orders: List[Order] = []

    bucket = choose_bucket(ctx, position)
    action = choose_action(bucket, ctx, position)

    if action == "passive_two_sided":
        buy_budget = post_passive_buy(
            orders,
            depth,
            fair,
            buy_budget,
            size_for_side(10, position, "buy"),
            min_edge=1.0,
            promote=False,
        )
        sell_budget = post_passive_sell(
            orders,
            depth,
            fair,
            sell_budget,
            size_for_side(10, position, "sell"),
            min_edge=1.0,
            promote=False,
        )
    elif action == "passive_buy":
        promote = bucket in {"bid_only", "cheap_buy", "cheap_buy_extreme", "recycle_short"}
        buy_budget = post_passive_buy(
            orders,
            depth,
            fair + 0.20,
            buy_budget,
            size_for_side(16, position, "buy"),
            min_edge=0.8,
            promote=promote,
        )
    elif action == "passive_sell":
        promote = bucket in {"ask_only", "rich_sell", "rich_sell_extreme", "recycle_long"}
        sell_budget = post_passive_sell(
            orders,
            depth,
            fair - 0.20,
            sell_budget,
            size_for_side(16, position, "sell"),
            min_edge=0.8,
            promote=promote,
        )
    elif action == "cross_buy":
        buy_budget, bought = post_cross_buy(orders, depth, fair, buy_budget, 10, edge=0.9)
        position += bought
        buy_budget = post_passive_buy(
            orders,
            depth,
        ...
        sell_budget, sold = post_cross_sell(orders, depth, fair, sell_budget, 10, edge=0.9)
        position -= sold
        sell_budget = post_passive_sell(
            orders,
            depth,
        ...
        sell_budget = post_passive_sell(
            ...
            fair - 0.10,
            sell_budget,
            size_for_side(14, position, "sell"),
            min_edge=0.8,
            promote=True,
        )
    elif bucket == "recycle_short":
        buy_budget = post_passive_buy(
            ...
            fair + 0.10,
            buy_budget,
            size_for_side(14, position, "buy"),
            min_edge=0.8,
            promote=True,
        )

    ash_state["last_state"] = bucket
    ash_state["last_action"] = action
