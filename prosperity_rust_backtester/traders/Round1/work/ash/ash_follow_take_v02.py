...
def trade_ash(
    ...
    buy_follow_take_signal = (
        buy_follow_signal
        and temp_position < max(42, soft_cap + 24)
        and (
            spread <= 11
            or (shock and top_imbalance >= 0.46 and micro_gap >= 0.46)
        )
    )
    ...
    def allow_buy_take_at(price: int) -> bool:
        ...
        if medium_buy_spread:
            if buy_follow_take_signal:
                follow_take_edge = take_edge - 0.25 if spread <= 10 else take_edge - 0.05
    ...
    def allow_sell_take_at(price: int) -> bool:
        ...
        if medium_sell_spread:
            if sell_follow_take_signal:
                follow_take_edge = take_edge + 0.20 if spread == 10 else take_edge - 0.15
                return edge >= follow_take_edge
            return inventory_emergency
        return True
