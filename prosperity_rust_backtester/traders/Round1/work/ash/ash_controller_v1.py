...
def trade_ash(
    ...
    buy_pressure = max(0.0, ctx["pressure"]) + 0.10 * ctx["buy_run"]
    sell_pressure = max(0.0, -ctx["pressure"]) + 0.10 * ctx["sell_run"]
    cheap_follow = ctx["anchor_dev"] <= -2.5 and buy_pressure >= 0.26
    rich_follow = ctx["anchor_dev"] >= 2.5 and sell_pressure >= 0.26

    if ctx["ask_only"] and position > -68:
        state = "ask_only_sell"
    elif ctx["bid_only"] and position < 68:
        state = "bid_only_buy"
    elif rich_follow and position > -52:
        state = "rich_follow_sell"
    elif cheap_follow and position < 52:
        state = "cheap_follow_buy"
    elif position >= 56 and not rich_follow and not ctx["ask_only"]:
        state = "recycle_long"
    elif position <= -56 and not cheap_follow and not ctx["bid_only"]:
        state = "recycle_short"
    else:
        state = "neutral_make"
    ...
    if state == "neutral_make":
        ...
        sell_budget = post_passive_sell(
            ...
            fair,
            sell_budget,
            size_for_side(11, position, "sell"),
            min_edge=1.0,
            promote=False,
        )
    elif state in {"bid_only_buy", "cheap_follow_buy"}:
        if state == "cheap_follow_buy" and ctx["anchor_dev"] <= -5.0 and buy_pressure >= 0.52 and position < 36:
            buy_budget, bought = post_cross_buy(orders, depth, fair, buy_budget, 8, edge=1.0)
            position += bought
        buy_budget = post_passive_buy(
            orders,
            depth,
            fair + 0.25,
            buy_budget,
            size_for_side(16 if state == "bid_only_buy" else 15, position, "buy"),
            min_edge=0.8,
            promote=True,
        )
        if position > 30 and ctx["anchor_dev"] >= 1.5 and buy_pressure < 0.35:
            sell_budget = post_passive_sell(
                orders,
                depth,
                fair,
    ...
    elif state in {"ask_only_sell", "rich_follow_sell"}:
        if state == "rich_follow_sell" and ctx["anchor_dev"] >= 5.0 and sell_pressure >= 0.52 and position > -36:
            sell_budget, sold = post_cross_sell(orders, depth, fair, sell_budget, 8, edge=1.0)
            position -= sold
        sell_budget = post_passive_sell(
            orders,
            depth,
            fair - 0.25,
            sell_budget,
            size_for_side(16 if state == "ask_only_sell" else 15, position, "sell"),
            min_edge=0.8,
            promote=True,
        )
        if position < -30 and ctx["anchor_dev"] <= -1.5 and sell_pressure < 0.35:
            buy_budget = post_passive_buy(
                orders,
                depth,
                fair,
        ...
        if position >= 70 and ctx["pressure"] > -0.10:
            sell_budget, sold = post_cross_sell(orders, depth, fair, sell_budget, 8, edge=0.8)
            position -= sold
        sell_budget = post_passive_sell(
            orders,
            depth,
            fair - 0.15,
            sell_budget,
            size_for_side(15, position, "sell"),
            min_edge=0.8,
            promote=True,
        )
    else:
        if position <= -70 and ctx["pressure"] < 0.10:
            buy_budget, bought = post_cross_buy(orders, depth, fair, buy_budget, 8, edge=0.8)
            position += bought
        buy_budget = post_passive_buy(
            orders,
            depth,
