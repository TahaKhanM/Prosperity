...
def trade_ash(
    ...
    calm_wide_size_signal = (
        16 <= spread <= 19
        and not shock
        and abs(current_mid - ASH_ANCHOR) <= 2.0
        and abs(top_imbalance) <= 0.20
        and abs(micro_gap) <= 1.0
    )
    ...
    if prefer_inside_buy:
        buy_sizes[0] += 1
    if calm_wide_size_signal and abs(temp_position) <= max(24, soft_cap + 10):
        buy_sizes[0] += 2
        sell_sizes[0] += 2
