...
PEPPER_WIDE_MIN_SPREAD = env_int("PEPPER_WIDE_MIN_SPREAD", 11)
PEPPER_WIDE_MIN_POS = env_int("PEPPER_WIDE_MIN_POS", 60)
PEPPER_WIDE_RESID_LOW = env_float("PEPPER_WIDE_RESID_LOW", -2.4)
PEPPER_WIDE_RESID_HIGH = env_float("PEPPER_WIDE_RESID_HIGH", 1.8)
PEPPER_WIDE_PROGRESS_START = env_float("PEPPER_WIDE_PROGRESS_START", 0.10)
PEPPER_WIDE_PROGRESS_END = env_float("PEPPER_WIDE_PROGRESS_END", 0.93)
PEPPER_WIDE_CARRY_LEFT = env_float("PEPPER_WIDE_CARRY_LEFT", 90.0)
PEPPER_MILD_RECYCLE_RESID = env_float("PEPPER_MILD_RECYCLE_RESID", 3.4)
PEPPER_MILD_RECYCLE_OFFSET = env_int("PEPPER_MILD_RECYCLE_OFFSET", 4)
PEPPER_FLOW_POSITIVE = env_float("PEPPER_FLOW_POSITIVE", 4.0)
PEPPER_FLOW_NEGATIVE = env_float("PEPPER_FLOW_NEGATIVE", -4.0)
PEPPER_WIDE_BASE_BUY = env_int("PEPPER_WIDE_BASE_BUY", 12)
PEPPER_WIDE_BIG_BUY = env_int("PEPPER_WIDE_BIG_BUY", 20)
PEPPER_WIDE_BASE_SELL = env_int("PEPPER_WIDE_BASE_SELL", 4)
PEPPER_WIDE_BIG_SELL = env_int("PEPPER_WIDE_BIG_SELL", 8)
PEPPER_WIDE_LOW_POS = env_int("PEPPER_WIDE_LOW_POS", 66)
...
def trade_pepper(
    ...
    direct_buy_take = (
        ask is not None
        and spread <= 5
        and temp_position <= 72
        and residual <= -4.5
        and carry_left >= 120.0
    )
    if direct_buy_take:
        before = buy_budget
        take_size = 24 if trade_ema >= PEPPER_FLOW_POSITIVE and temp_position <= 68 else 16
        buy_budget = add_buy(orders, PEPPER, ask, take_size, buy_budget)
        temp_position += before - buy_budget

    direct_sell_take = (
        bid is not None
        and spread <= 5
        and temp_position >= 70
        and 0.12 <= progress <= 0.92
        and residual >= PEPPER_MILD_RECYCLE_RESID
    )
    if direct_sell_take:
        before = sell_budget
        take_size = 24 if trade_ema >= PEPPER_FLOW_POSITIVE and temp_position >= 72 else 12
        sell_budget = add_sell(orders, PEPPER, bid, take_size, sell_budget)
        temp_position -= before - sell_budget
    ...
    if wide_carry_mode:
        buy_prices = [bid + 1, bid]
        sell_prices = [ask - 1, ask]
        if temp_position >= 74:
            buy_sizes = [
                PEPPER_WIDE_BASE_BUY if temp_position <= 76 else max(6, PEPPER_WIDE_BASE_BUY - 4),
                max(4, PEPPER_WIDE_BASE_BUY - 6),
            ]
            sell_sizes = [PEPPER_WIDE_BIG_SELL, max(3, PEPPER_WIDE_BASE_SELL)]
        elif trade_ema >= PEPPER_FLOW_POSITIVE:
            buy_sizes = [PEPPER_WIDE_BIG_BUY, max(6, PEPPER_WIDE_BASE_BUY)]
            sell_sizes = [PEPPER_WIDE_BIG_SELL, max(3, PEPPER_WIDE_BASE_SELL)]
            ...
            sell_sizes = [PEPPER_WIDE_BASE_SELL, max(2, PEPPER_WIDE_BASE_SELL - 2)]
        else:
            buy_sizes = [
                PEPPER_WIDE_BASE_BUY if temp_position <= 72 else max(8, PEPPER_WIDE_BASE_BUY - 4),
                max(4, PEPPER_WIDE_BASE_BUY - 6),
