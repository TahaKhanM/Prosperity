...
def trade_pepper(
    ...
    flow_now = diagnostics["flow_now"]
    market_trade_qty = sum(abs(int(trade.quantity)) for trade in market_trades)
    ...
    wide_carry_mode = (
        ...
        and ask is not None
        and spread >= 6
        and market_trade_qty >= 2
        and PEPPER_WIDE_PROGRESS_START <= progress <= PEPPER_WIDE_PROGRESS_END
        and carry_left >= PEPPER_WIDE_CARRY_LEFT
        and PEPPER_WIDE_RESID_LOW <= residual <= 2.6
        and temp_position >= 56
    )
    if wide_carry_mode:
        buy_prices = [bid + 1, bid]
        sell_prices = [ask - 1, ask]
        if temp_position >= 72:
            buy_sizes = [max(4, PEPPER_WIDE_BASE_BUY - 4), max(2, PEPPER_WIDE_BASE_BUY - 8)]
            sell_sizes = [PEPPER_WIDE_BIG_SELL, max(2, PEPPER_WIDE_BASE_SELL)]
        elif flow_now >= 4.0:
            buy_sizes = [PEPPER_WIDE_BIG_BUY, max(6, PEPPER_WIDE_BASE_BUY)]
            sell_sizes = [PEPPER_WIDE_BIG_SELL, max(3, PEPPER_WIDE_BASE_SELL)]
        elif flow_now <= -4.0:
            buy_sizes = [PEPPER_WIDE_BASE_BUY, max(4, PEPPER_WIDE_BASE_BUY - 4)]
