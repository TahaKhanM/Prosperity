...
def trade_ash(
    ...
    buy_size = 12 if not shock else 10
    sell_size = 12 if not shock else 10
    if spread >= 20 and not shock:
        buy_size = 14
        sell_size = 14

    if position > 42:
        buy_size = max(6, buy_size - 4)
        sell_size += 4
    elif position < -42:
        buy_size += 4
        sell_size = max(6, sell_size - 4)

    buy_price: Optional[int] = None
    if prefer_inside and bid is not None and ask is not None:
        inside_bid = bid + 1
        if inside_bid < ask and fair - inside_bid >= 1.0:
            buy_price = inside_bid
    if buy_price is None and position < -36 and bids_below_fair:
        fallback_bid = max(bids_below_fair)
        if fair - fallback_bid >= 1.2:
            buy_price = fallback_bid
            buy_size = min(buy_size, 10)

    sell_price: Optional[int] = None
    if prefer_inside and bid is not None and ask is not None:
        inside_ask = ask - 1
        if inside_ask > bid and inside_ask - fair >= 1.0:
            sell_price = inside_ask
    if sell_price is None and position > 36 and asks_above_fair:
        fallback_ask = min(asks_above_fair)
        if fallback_ask - fair >= 1.2:
            sell_price = fallback_ask
            sell_size = min(sell_size, 10)

    if buy_price is not None and (ask is None or buy_price < ask):
        buy_budget = add_buy(orders, ASH, buy_price, buy_size, buy_budget)

    if sell_price is not None and (bid is None or sell_price > bid):
        sell_budget = add_sell(orders, ASH, sell_price, sell_size, sell_budget)
