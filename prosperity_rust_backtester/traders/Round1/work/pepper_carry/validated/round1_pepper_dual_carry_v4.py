...
def trade_pepper(
    ...
    passive_carry_mode = (
        bid is not None
        and ask is not None
        and not wide_carry_mode
        and 6 <= spread <= 10
        and market_trade_qty >= 2
        and 0.10 <= progress <= 0.93
        and carry_left >= 100.0
        and -1.5 <= residual <= 1.5
        and 56 <= temp_position <= 72
        and trade_ema > -4.0
    )
    if wide_carry_mode:
        buy_prices[0] = bid + 1
        ...
        if temp_position >= 76:
            sell_sizes[0] = max(sell_sizes[0], PEPPER_WIDE_BIG_SELL)
    elif passive_carry_mode:
        buy_prices[0] = bid + 1
        sell_prices[0] = ask - 1
        buy_sizes[0] = max(buy_sizes[0], 12)
        buy_sizes[1] = max(buy_sizes[1], 8)
        sell_sizes[0] = max(sell_sizes[0], 4 if temp_position < 68 else 6)
        sell_sizes[1] = max(sell_sizes[1], 2)
