...
def trade_pepper(
    ...
    current_spread = (top_ask - top_bid) if top_bid is not None and top_ask is not None else 0
    ...
    suppress_tight_recycle = mild_recycle and current_spread <= 4 and abs(trade_ema) < 1.0
    for bid in sorted(depth.buy_orders, reverse=True):
        if suppress_tight_recycle:
            break
