...
def trade_pepper(
    ...
    suppress_high_inventory_buy = high_inventory_buy_hazard and not allow_high_inventory_buy
    ...
    if quote_buy_hazard:
        bid_edge += 0.8
        buy_sizes[0] = min(buy_sizes[0], 6)
        buy_sizes[1] = min(buy_sizes[1], 3)
    suppress_passive_buy = quote_buy_hazard and (
        carry_left < 220.0
        or residual > 1.5
        or top_imbalance <= PEPPER_BOOK_LEAN_POSITIVE
    )
