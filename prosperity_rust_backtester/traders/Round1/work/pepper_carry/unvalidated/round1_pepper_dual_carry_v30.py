...
def depth_ratio(depth: OrderDepth, levels: int = 3) -> float:
    bid_total = sum(
        int(volume)
        for _, volume in sorted(depth.buy_orders.items(), reverse=True)[:levels]
    )
    ask_total = sum(
        int(-volume)
        for _, volume in sorted(depth.sell_orders.items())[:levels]
    )
...
def trade_pepper(
    ...
    top3_depth_ratio = depth_ratio(depth, 3)
