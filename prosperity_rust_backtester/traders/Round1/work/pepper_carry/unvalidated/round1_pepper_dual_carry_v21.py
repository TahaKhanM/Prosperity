...
PEPPER_TIGHT_SPREAD_MAX = 6
PEPPER_TIGHT_BUY_POS_MIN = 56
PEPPER_TIGHT_BUY_POS_MAX = 72
PEPPER_TIGHT_SELL_POS_MIN = 72
...
def trade_pepper(
    ...
    tight_buy_exact_mode = (
        PEPPER_TIGHT_BUY_POS_MIN <= position <= PEPPER_TIGHT_BUY_POS_MAX
        and spread <= PEPPER_TIGHT_SPREAD_MAX
        and top_imbalance >= PEPPER_BOOK_LEAN_POSITIVE
        and not sellflow
        and residual <= 2.0
    )
    tight_sell_exact_mode = (
        position >= PEPPER_TIGHT_SELL_POS_MIN
        and spread <= PEPPER_TIGHT_SPREAD_MAX
        and top_imbalance <= PEPPER_BOOK_LEAN_NEGATIVE
        and not buyflow
        and residual >= -0.5
    )
    if tight_buy_exact_mode:
        long_cap = 80
        target_long = 80
        buy_edge = max(0.7, buy_edge - 0.35)
        sweep_pad += 1.0
        bid_edge = max(1.0, bid_edge - 0.2)
        bid_size += 2
    if tight_sell_exact_mode:
        target_long = max(72, target_long - 3)
        sell_edge = max(3.0, sell_edge - 1.0)
        ask_edge = max(4.0, ask_edge - 0.7)
        ask_size += 2
    ...
    if best_ask_level is not None:
        ...
        if tight_buy_exact_mode:
            buy_ceiling = max(buy_ceiling, template_fair + 1.0)
    ...
    if tight_sell_exact_mode:
        recycle_floor = min(recycle_floor, 72)
    best_bid_level = best_bid(depth)
    if best_bid_level is not None:
        ...
        if tight_sell_exact_mode:
            sell_threshold = min(sell_threshold, template_fair + 0.5)
