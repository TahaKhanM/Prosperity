...
def trade_pepper(
    ...
    if wide_carry_mode:
        ...
        if carry_buy_pressure:
            buy_sizes[0] = max(
                buy_sizes[0],
                20 if temp_position <= 72 else 16,
            )
            if temp_position <= 68 and carry_left >= 180.0:
                buy_sizes[0] = max(buy_sizes[0], 22)
            if not sellflow:
                sell_sizes[0] = max(1, min(sell_sizes[0], 3))
        elif carry_sell_pressure:
            buy_sizes[0] = max(6, buy_sizes[0] - 4)
            sell_sizes[0] = max(
                sell_sizes[0],
                10 if temp_position >= 76 else 8,
            )
