...
PEPPER_REBUY_COOLDOWN_SHORT = 200
PEPPER_REBUY_COOLDOWN_LONG = 400
PEPPER_REBUY_COOLDOWN_MIN_POS = 78
PEPPER_REBUY_COOLDOWN_QUOTE_POS = 76
...
def trade_pepper(
    ...
    cooldown_until = int(pepper_state.get("rebuy_cooldown_until", -1))
    recent_sell_qty = sum(
        int(trade.quantity)
    ...
    if (
        recent_sell_qty >= PEPPER_WIDE_BASE_SELL
        and spread >= PEPPER_HAZARD_MIN_SPREAD
        and PEPPER_HAZARD_PROGRESS_START <= progress <= PEPPER_HAZARD_PROGRESS_END
        and position >= PEPPER_REBUY_COOLDOWN_MIN_POS
    ):
        cooldown_until = max(
            cooldown_until,
            timestamp + (
                PEPPER_REBUY_COOLDOWN_LONG
                if recent_sell_qty >= PEPPER_WIDE_BIG_SELL
                else PEPPER_REBUY_COOLDOWN_SHORT
            ),
        )
    ...
    suppress_high_inventory_buy = high_inventory_buy_hazard or (
        ...
        and spread >= PEPPER_HAZARD_MIN_SPREAD
        and position >= PEPPER_REBUY_COOLDOWN_MIN_POS
        and not allow_high_inventory_buy
    )
    ...
    quote_buy_hazard = (
        ...
        and spread >= PEPPER_HAZARD_MIN_SPREAD
        and PEPPER_HAZARD_PROGRESS_START <= progress <= PEPPER_HAZARD_PROGRESS_END
        and temp_position >= PEPPER_REBUY_COOLDOWN_QUOTE_POS
        and not allow_high_inventory_buy
    )
    if wide_carry_mode:
        buy_prices[0] = bid + 1
    ...
    pepper_state["rebuy_cooldown_until"] = cooldown_until
    ...
    def run(self, state: TradingState):
        ...
        if PEPPER in state.order_depths:
            result[PEPPER], pepper_state = trade_pepper(
                ...
                list(state.own_trades.get(PEPPER, [])),
                int(state.timestamp),
            )
