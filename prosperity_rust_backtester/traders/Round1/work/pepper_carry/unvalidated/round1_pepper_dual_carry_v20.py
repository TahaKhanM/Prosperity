...
def trade_pepper(
    ...
    carry_template = (
        wide_carry_mode
        and abs(temp_position - target_long) <= 6
        and abs(top_imbalance) < 0.18
    ...
    bid_only_template = (
        ...
        and (
            mode in ("opening", "discount")
            or carry_buy_pressure
            or buyflow
            or (top_imbalance >= PEPPER_BOOK_LEAN_POSITIVE and residual <= 1.5)
        )
    )
    ask_only_template = (
        temp_position > max(72, target_long - 4)
        and (
            recycle_mode
            or mild_recycle
            or carry_sell_pressure
            or sellflow
            or (mode == "overheated" and not buyflow)
        )
    )
    if carry_template and top_imbalance <= PEPPER_BOOK_LEAN_NEGATIVE and not buyflow:
        carry_template = False
        ask_only_template = True
    if carry_template and top_imbalance >= PEPPER_BOOK_LEAN_POSITIVE and temp_position <= target_long - 2:
        carry_template = False
        bid_only_template = True

    quote_template = "hold"
    if carry_template:
        quote_template = "carry_two_sided"
    elif bid_only_template and ask_only_template:
        quote_template = "carry_two_sided"
    elif bid_only_template:
        quote_template = "bid_only"
    elif ask_only_template:
        quote_template = "ask_only"

    seen = set()
    buy_plan = list(zip(buy_prices, buy_sizes))[:1]
    if quote_template in {"bid_only", "carry_two_sided"}:
        for idx, (price, size) in enumerate(buy_plan):
            if bid is not None and idx == 0:
                price = min(price, bid + 1)
            if ask is not None and price >= ask:
    ...
    if quote_template in {"ask_only", "carry_two_sided"}:
        seen.clear()
