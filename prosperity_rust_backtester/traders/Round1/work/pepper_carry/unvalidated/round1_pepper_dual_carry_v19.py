...
PEPPER_LEAN_STREAK_SOFT = 2
PEPPER_LEAN_STREAK_HARD = 4
PEPPER_LEAN_BUY_SUPPRESS_POS = 70
PEPPER_LEAN_BUY_RELIEF_RESID = -3.0
PEPPER_LEAN_SELL_RELIEF_RESID = 4.5
...
def pepper_state_and_fair(
    ...
    current_bid = best_bid(depth)
    current_ask = best_ask(depth)
    top_imbalance = 0.0
    if current_bid is not None and current_ask is not None:
        bid_volume = int(depth.buy_orders[current_bid])
        ask_volume = int(-depth.sell_orders[current_ask])
        total_top = bid_volume + ask_volume
        if total_top > 0:
            top_imbalance = (bid_volume - ask_volume) / total_top

    lean_streak = int(pepper_state.get("lean_streak", 0))
    if top_imbalance >= PEPPER_BOOK_LEAN_POSITIVE:
        lean_streak = lean_streak + 1 if lean_streak >= 0 else 1
    elif top_imbalance <= PEPPER_BOOK_LEAN_NEGATIVE:
        lean_streak = lean_streak - 1 if lean_streak <= 0 else -1
    elif lean_streak > 0:
        lean_streak = max(0, lean_streak - 1)
    elif lean_streak < 0:
        lean_streak = min(0, lean_streak + 1)
    ...
    pepper_state["lean_streak"] = lean_streak
...
def trade_pepper(
    ...
    lean_streak = int(diagnostics.get("lean_streak", 0.0))
    ...
    if bid is not None and ask is not None:
        ...
        if total_top > 0:
            top_imbalance = (bid_volume - ask_volume) / total_top
    bullish_lean = lean_streak >= PEPPER_LEAN_STREAK_SOFT
    bearish_lean = lean_streak <= -PEPPER_LEAN_STREAK_SOFT
    strong_bullish_lean = lean_streak >= PEPPER_LEAN_STREAK_HARD
    strong_bearish_lean = lean_streak <= -PEPPER_LEAN_STREAK_HARD
    ...
    if bullish_lean and not sellflow:
        target_long = min(long_cap, target_long + (2 if strong_bullish_lean else 1))
        buy_edge = max(0.8, buy_edge - (0.25 if strong_bullish_lean else 0.1))
        bid_edge = max(0.9, bid_edge - (0.2 if strong_bullish_lean else 0.1))
        ask_size = max(1, ask_size - 1)
    elif bearish_lean and not buyflow:
        target_long = max(72, target_long - (4 if strong_bearish_lean else 2))
        buy_edge += 0.2 if strong_bearish_lean else 0.1
        sell_edge = max(3.2, sell_edge - (0.7 if strong_bearish_lean else 0.35))
        ask_edge = max(4.3, ask_edge - (0.6 if strong_bearish_lean else 0.25))
        bid_size = max(6, bid_size - (3 if strong_bearish_lean else 1))
        ask_size += 1 if strong_bearish_lean else 0
    ...
    if wide_carry_mode:
        ...
        if strong_bearish_lean and not carry_buy_pressure and not buyflow:
            buy_prices[0] = bid
        if strong_bullish_lean and not sellflow and temp_position <= target_long:
            sell_sizes[0] = 1

    suppress_passive_buy = (
        bearish_lean
        and not buyflow
        and not carry_buy_pressure
        and residual > PEPPER_LEAN_BUY_RELIEF_RESID
        and temp_position >= PEPPER_LEAN_BUY_SUPPRESS_POS
    )
    suppress_passive_sell = (
        bullish_lean
        and not recycle_mode
        and not mild_recycle
        and not sellflow
        and residual < PEPPER_LEAN_SELL_RELIEF_RESID
        and temp_position <= target_long
    )

    seen = set()
    buy_plan = list(zip(buy_prices, buy_sizes))[:1]
    if not suppress_passive_buy:
        for idx, (price, size) in enumerate(buy_plan):
            if bid is not None and idx == 0:
                price = min(price, bid + 1)
            if ask is not None and price >= ask:
