...
PEPPER_LEANED_RECYCLE_SHIFT = 1.2
...
def trade_pepper(
    ...
    if carry_buy_pressure:
        ...
        target_long = max(target_long, 80 if residual <= 0.8 and carry_left >= 180.0 else 79)
        buy_edge = max(0.8, buy_edge - 0.15)
        bid_edge = max(1.0, bid_edge - 0.15)
        bid_size += 2
        if not sellflow:
            ask_size = max(1, ask_size - 1)
    elif carry_sell_pressure and not buyflow:
        target_long = max(72, target_long - (4 if residual >= 2.0 else 2))
        sell_edge = max(3.4, sell_edge - 0.35)
        ask_edge = max(4.8, ask_edge - 0.45)
