...
TOMATO_EXTRA_BUY_EDGE = 1.25
TOMATO_EXTRA_BUY_POS_CAP = 70
TOMATO_INV_SKEW = 0.0
TOMATO_WARNING = 80
TOMATO_SOFT_CAP = 80
...
def add_buy(
    ...
    if size > 0:
        add_signed_order(orders, symbol, int(price), size)
        buy_budget -= size
    return buy_budget
...
def passive_sell_edge(position: int) -> float:
    if position >= TOMATO_WARNING:
        return TOMATO_REDUCE_EDGE
    return TOMATO_PASSIVE_SELL_EDGE


def passive_buy_edge(position: int) -> float:
    if position <= -TOMATO_WARNING:
        return TOMATO_REDUCE_EDGE
    return TOMATO_PASSIVE_BUY_EDGE
...
def trade_tomatoes(depth: OrderDepth, position: int, recent_mids: List[float]) -> List[Order]:
    ...
    if (
        ...
        and passive_bid < ask
        and fair - passive_bid >= passive_buy_edge(temp_position)
    ):
        before = buy_budget
        buy_budget = add_buy(
    ...
    if (
        temp_position > -TOMATO_SOFT_CAP
        and passive_ask > bid
        and passive_ask - fair >= passive_sell_edge(temp_position)
    ):
        before = sell_budget
    ...
    if (
        spread >= 13
        and temp_position < TOMATO_EXTRA_BUY_POS_CAP
