...
def top_imbalance(depth: OrderDepth) -> float:
    ...
    if bid is None or ask is None:
        return 0.0
    bid_volume = max(0, int(depth.buy_orders.get(bid, 0)))
    ask_volume = max(0, int(-depth.sell_orders.get(ask, 0)))
    total = bid_volume + ask_volume
    if total <= 0:
        return 0.0
...
def trade_pepper(
    ...
    if imbalance > 0.20:
        ...
        if residual >= 2.0:
            target_long = max(72, target_long - 2)
            sell_edge = max(3.4, sell_edge - 0.7)
            ask_edge = max(4.8, ask_edge - 0.4)
            ask_size += 1
        elif residual <= -2.0:
            buy_edge += 0.15
            bid_size = max(8, bid_size - 1)
